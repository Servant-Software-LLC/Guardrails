# catches: a transport that reports {0,0} usage instead of null, classifies a 404 model-not-found as Transient (burning the 4h pause budget waiting for a human action no waiting produces), bounds the context once at entry instead of per turn, or does not stream (leaving the log viewer showing a dead file for the whole attempt).
#
...
#          Verified discriminating: 'OpenAiCompatTransportTests' is a substring of no other test class in this plan or in
#          the target project.
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "OpenAiCompatTransportTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "OpenAiCompatTransportTests green: $executed executed, 0 failed."
exit 0
