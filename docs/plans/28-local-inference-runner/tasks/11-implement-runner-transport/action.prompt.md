...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "11-implement-runner-transport": { "someKey": "someValue" } }`.
...
stay red until tasks 11 and 12 - expected, and your guardrail filters to your own class.
...
- **The request.** POST to `{endpoint}/chat/completions` with `model`, `messages`, `stream: true`,
  ...
  harness-owned field (a validation error in task 19; the class refuses too, as backstop).
...
- **The context bounds (section 6.1), both halves.** Refuse before sending when
  ...
  and the attempt fails with `PromptFailureKind.ContextOverflow`, actionable feedback, and no
