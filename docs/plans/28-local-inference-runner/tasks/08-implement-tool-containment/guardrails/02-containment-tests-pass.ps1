# catches: a containment primitive that repeats WorkspaceContainment's mistake (rejecting every rooted path, which would refuse every read the harness instructs the model to make), or that accepts a prefix match instead of a directory-boundary match, or that fails OPEN on an empty root set.
#
...
#          Verified discriminating: 'PromptToolContainmentTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~PromptToolContainmentTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "PromptToolContainmentTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "PromptToolContainmentTests green: $executed executed, 0 failed."
exit 0
