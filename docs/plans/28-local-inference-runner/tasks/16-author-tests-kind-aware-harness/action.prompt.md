...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "16-author-tests-kind-aware-harness": { "someKey": "someValue" } }`.
...
Drive the **real** `GuardrailRunner`, the **real** `PromptComposer` and the **real**
`OpenAiCompatPromptRunner` against the `FakeOpenAiServer`. The only thing faked is the HTTP endpoint,
which is a process boundary - do NOT substitute an in-process double for any of the three.
