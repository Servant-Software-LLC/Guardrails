...
#          business touching. Scoped to the existing journal suite, which this task edits shared code in - not the whole
...
# exempts. Measured at authoring time: the filter selects 68 existing tests, all passing.
$ErrorActionPreference = 'Continue'

$env:DOTNET_CLI_UI_LANGUAGE = 'en'

$project = 'tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj'
$filter = 'FullyQualifiedName~Journal'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "This task broke the existing journal suite ($failed failed of $executed). Its own tests passing is not enough - fix the regression."
    exit 1
}
...
Write-Output "Neighbours intact: $executed executed in the existing journal suite, 0 failed."
exit 0
