# catches: recording judge spend as 0 instead of null when the runner reports none, or FOLDING it into JournalCost.Total - which would make maxCostUsd trip earlier on every existing Claude run and silently change the --autonomous brake, a semantic change to the liveness floor shipped inside a local-inference plan.
#
...
#          Verified discriminating: 'JudgeSpendRecordingTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~JudgeSpendRecordingTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "JudgeSpendRecordingTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "JudgeSpendRecordingTests green: $executed executed, 0 failed."
exit 0
