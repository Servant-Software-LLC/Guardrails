# catches: a splice that still fires for a runner with no write tools (breaking every worktree-mode pinned judge - the flagship deliverable in the default execution mode), or a composer that hands a non-writing runner the write-your-verdict instruction it cannot follow.
#
...
#          Verified discriminating: 'KindAwareHarnessTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~KindAwareHarnessTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "KindAwareHarnessTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "KindAwareHarnessTests green: $executed executed, 0 failed."
exit 0
