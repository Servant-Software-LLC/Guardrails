...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "14-author-tests-block-diagnostics": { "someKey": "someValue" } }`.
...
- an `openai-compat` block declaring no `strength`. This matters mechanically:
  ...
  every judge on it carries a advisory forever, and advisories that always fire stop being read.
...
All must FAIL today. **Do NOT implement them** - that is task 15.
