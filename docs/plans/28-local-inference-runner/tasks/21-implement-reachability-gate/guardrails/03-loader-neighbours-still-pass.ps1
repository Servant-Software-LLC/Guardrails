...
#          business touching. Scoped to the existing loader suite, which this task edits shared code in - not the whole
...
# exempts. Measured at authoring time: the filter selects 44 existing tests, all passing.
$ErrorActionPreference = 'Continue'

$env:DOTNET_CLI_UI_LANGUAGE = 'en'

$project = 'tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj'
$filter = 'FullyQualifiedName~Loader'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "This task broke the existing loader suite ($failed failed of $executed). Its own tests passing is not enough - fix the regression."
    exit 1
}
...
Write-Output "Neighbours intact: $executed executed in the existing loader suite, 0 failed."
exit 0
