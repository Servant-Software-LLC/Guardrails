# catches: a gate that misses a route - especially route 2's sole-declared-runner half, which no default pointer names and which fires on the most natural misconfiguration there is - or one that fires on the two LEGAL paths (a judge's frontmatter pin, an Advisory reserved profile name), which would make v1's flagship deliverable unreachable.
#
...
#          Verified discriminating: 'ActionReachabilityGateTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~ActionReachabilityGateTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "ActionReachabilityGateTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "ActionReachabilityGateTests green: $executed executed, 0 failed."
exit 0
