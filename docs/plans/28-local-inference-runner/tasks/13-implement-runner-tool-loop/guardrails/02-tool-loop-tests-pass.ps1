# catches: a tool loop that names its tools anything other than Read/Glob/Grep (contradicting the harness's own supervisory prompt text), skips containment, ignores the consecutive-denial bound, or - the false green this plan exists to close - lets a Guardrail-role invocation that made ZERO tool calls produce a passing verdict.
#
...
#          Verified discriminating: 'OpenAiCompatToolLoopTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~OpenAiCompatToolLoopTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "OpenAiCompatToolLoopTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "OpenAiCompatToolLoopTests green: $executed executed, 0 failed."
exit 0
