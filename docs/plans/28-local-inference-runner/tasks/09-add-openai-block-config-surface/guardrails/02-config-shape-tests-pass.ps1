# catches: block keys declared on the record but never BOUND by the loader - the properties exist, the JSON round-trip drops them, and every downstream task builds against a config surface that silently reads null. Also a key given a non-null default, which would change every existing guardrails.json.
#
...
#          Verified discriminating: 'OpenAiCompatConfigShapeTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~OpenAiCompatConfigShapeTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "OpenAiCompatConfigShapeTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "OpenAiCompatConfigShapeTests green: $executed executed, 0 failed."
exit 0
