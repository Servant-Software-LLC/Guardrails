...
#          business touching. Scoped to the providers-init suite (whose ModelEnumerable pin this task RE-BASELINES), which this task edits shared code in - not the whole
...
# exempts. Measured at authoring time: the filter selects 15 existing tests, all passing.
$ErrorActionPreference = 'Continue'

$env:DOTNET_CLI_UI_LANGUAGE = 'en'

$project = 'tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj'
$filter = 'FullyQualifiedName~ProvidersInit'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "This task broke the providers-init suite (whose ModelEnumerable pin this task RE-BASELINES) ($failed failed of $executed). Its own tests passing is not enough - fix the regression."
    exit 1
}
...
Write-Output "Neighbours intact: $executed executed in the providers-init suite (whose ModelEnumerable pin this task RE-BASELINES), 0 failed."
exit 0
