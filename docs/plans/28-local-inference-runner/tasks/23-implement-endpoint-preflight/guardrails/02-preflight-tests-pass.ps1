# catches: a preflight that halts on a 404 from /models (locking out an engine that serves chat perfectly), that skips the tool-capability probe, or that lets a 200-with-no-tool_calls through - which is the exact silent case that produces a verifier certifying work it never read.
#
...
#          Verified discriminating: 'OpenAiCompatPreflightTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~OpenAiCompatPreflightTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "OpenAiCompatPreflightTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "OpenAiCompatPreflightTests green: $executed executed, 0 failed."
exit 0
