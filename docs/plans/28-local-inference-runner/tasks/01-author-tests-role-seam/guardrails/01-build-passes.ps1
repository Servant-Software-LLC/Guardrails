...
#          guardrail on this task is meaningless over a tree that does not build, and for this task the build IS the gate: making PromptInvocation.Role `required` is a source break at every construction site including every test fixture, and a tree that compiles is the proof every site was found - including CriticalityJudge's target-typed new(), which a grep for 'new PromptInvocation' does not match (verified: that grep returns 6, not 7).
