# catches: a check that collapses 'unknown' into 'unmet' (making the report lie about what it could not determine), or that exits non-zero on an unmet assumption - it is a report, not a gate.
#
...
#          Verified discriminating: 'ProvidersCheckTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~ProvidersCheckTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "ProvidersCheckTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "ProvidersCheckTests green: $executed executed, 0 failed."
exit 0
