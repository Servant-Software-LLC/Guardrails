...
#          guardrail on this task is meaningless over a tree that does not build, and for this task the stub must make the test project COMPILE - a non-compiling test file exits non-zero identically to a failing one, so without this the red proof is worthless.
