...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "20-author-tests-reachability-gate": { "someKey": "someValue" } }`.
...
routes and **each needs its own test** - a single combined test would let a route regress unnoticed:
...
4. an **action prompt's YAML frontmatter `runner:`** names it - the route the validator cannot see
   today, requiring task 21's loader fold
5. it is declared under a reserved Action-role profile name (`ai-merge` or `breakdown`)

And the **negatives**, which keep the gate from being a blunt ban: an `openai-compat` block pinned by
a **judge guardrail's** frontmatter `runner:` is LEGAL and must NOT fire, and so is one named
`overwatch` or `ai-triage` (the Advisory reserved profiles). Those two are the entire point of v1 - if
the gate fires on them, the flagship deliverable is unreachable.
