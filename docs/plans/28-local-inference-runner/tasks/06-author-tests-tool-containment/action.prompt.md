...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "06-author-tests-tool-containment": { "someKey": "someValue" } }`.
...
- **EMPTY entries are dropped** from the root set (`Path.GetFullPath("")` throws, and
  `CriticalityJudge` supplies two empty ones - this is a real caller, not a hypothetical)
...
All must FAIL against the throwing stub. **Do NOT implement it** - that is task 07.
