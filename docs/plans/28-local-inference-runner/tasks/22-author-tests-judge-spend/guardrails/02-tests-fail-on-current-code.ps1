...
if ($failed -lt 2) {
    Write-Output "EXPECTED AT LEAST 2 FAILING TEST(S), saw $failed (of $executed executed). These tests are supposed to FAIL against the stubs - a green suite here means they are not bound to the not-yet-written behaviour, which is a tautology that would let the implementation task pass by doing nothing."
    exit 1
}
