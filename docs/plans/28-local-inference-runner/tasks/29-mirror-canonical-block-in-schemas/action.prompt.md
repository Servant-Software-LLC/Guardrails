...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "29-mirror-canonical-block-in-schemas": { "someKey": "someValue" } }`.
...
Mirror task 28's canonical `promptRunners` block into
`.claude/skills/plan-breakdown/references/schemas.md`, **byte for byte**.

The two are bound by the `canonical-schema:promptRunners` sentinel and the **existing
`SchemaDriftTests.PromptRunnersSchema_SkillCopyMatchesSsot`** test, which extracts the region from
both files and asserts they match. That test is your guardrail, so this is not a paraphrase and not a
summary - the block must match `docs/plans/02-schemas-and-contracts.md` character for character,
including comments, the leading indent, and the absent (`null`) states of `endpoint`,
