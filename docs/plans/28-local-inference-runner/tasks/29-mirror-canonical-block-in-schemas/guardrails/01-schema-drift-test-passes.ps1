# catches: a mirror that PARAPHRASES the canonical promptRunners block instead of reproducing it. This runs the repo's OWN drift test - SchemaDriftTests.PromptRunnersSchema_SkillCopyMatchesSsot already extracts the region from both files and asserts they match byte-for-byte. Re-implementing that comparison in PowerShell would put a SECOND extractor on one property, and two extractors on one property is the drift trap itself (#468 demotion: a test already carries this).
#
...
#          Verified discriminating: 'SchemaDriftTests' is a substring of no other test class in this plan or in
#          the target project.
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "SchemaDriftTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "SchemaDriftTests green: $executed executed, 0 failed."
exit 0
