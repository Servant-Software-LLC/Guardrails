...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "17-implement-reachability-gate": { "someKey": "someValue" } }`.
...
**1. Make route 5 visible - EXTRACT the frontmatter reader, never copy it.** `ActionRunner` resolves
...
**Extract, never copy.** A fourth independent frontmatter parser is how the two sites drift - and your
guardrail counts the parsers, so a copy will fail it.

**2. Implement GR2066** over all five routes, in `PlanValidator`, allocating the code in
`DiagnosticCodes.cs` (task 15 already advanced the marker; GR2066 is yours).
