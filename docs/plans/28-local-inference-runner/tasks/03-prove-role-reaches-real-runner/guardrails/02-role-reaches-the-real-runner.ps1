# catches: seven role assertions that all pass against a FAKED IPromptRunner while the role never survives the trip to the real one. The seam ledger classifies the producers-to-IPromptRunner rows as E (ClaudePromptRunner spawns the claude child process), and an E row owes a proof driving the real adapter with the PROCESS boundary faked underneath - a stub claude binary, which this repo already has fixtures for. Without this the whole role seam is certified by a double of the thing under test.
#
...
#          Verified discriminating: 'RoleReachesRealRunnerTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~RoleReachesRealRunnerTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "RoleReachesRealRunnerTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "RoleReachesRealRunnerTests green: $executed executed, 0 failed."
exit 0
