...
#          guardrail on this task is meaningless over a tree that does not build, and for this task the runner stub must make all THREE test classes compile - each later implementation task filters to one of them, so a class that does not compile silently removes a whole implementation task's gate.
