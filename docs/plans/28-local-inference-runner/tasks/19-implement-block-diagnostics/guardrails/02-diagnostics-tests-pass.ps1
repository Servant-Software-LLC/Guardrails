# catches: a validator that misses a malformed openai-compat block, or keeps drawing the GR2009 PATH warning on an endpoint URL - a warning that is always wrong trains people to ignore GR2009 entirely.
#
...
#          Verified discriminating: 'OpenAiCompatDiagnosticsTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~OpenAiCompatDiagnosticsTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "OpenAiCompatDiagnosticsTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "OpenAiCompatDiagnosticsTests green: $executed executed, 0 failed."
exit 0
