...
#          business touching. Scoped to the existing validator suite, which this task edits shared code in - not the whole
...
# exempts. Measured at authoring time: the filter selects 93 existing tests, all passing.
$ErrorActionPreference = 'Continue'

$env:DOTNET_CLI_UI_LANGUAGE = 'en'

$project = 'tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj'
$filter = 'FullyQualifiedName~Validator'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "This task broke the existing validator suite ($failed failed of $executed). Its own tests passing is not enough - fix the regression."
    exit 1
}
...
Write-Output "Neighbours intact: $executed executed in the existing validator suite, 0 failed."
exit 0
