...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "05-build-fake-openai-server": { "someKey": "someValue" } }`.
...
filters on it): a self-test proving the server is drivable end to end. At minimum, assert that a
scripted normal completion is received by a real `HttpClient` over the loopback socket, that a
scripted 404 arrives as a 404, and that the accepted-connection counter reports what actually
happened. This is test infrastructure with no behaviour of its own to TDD, so it is not split into a
pair - but a fixture nobody ever ran is a fixture three later task pairs will discover is broken.
