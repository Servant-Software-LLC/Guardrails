...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "24-record-openai-compat-in-ssot": { "someKey": "someValue" } }`.
...
code (invariant 4), and every one of them describes behaviour that tasks 01-23 have now shipped -
read the real code, not this plan's quotations of it, before writing.

In `docs/plans/02-schemas-and-contracts.md`:

1. **Section 2, the canonical `promptRunners` block.** Add `endpoint`, `contextTokens`, `apiKeyEnv`,
   `wire` and `engine`, **each in its absent (`null`) state**, noting they apply to
   `kind: "openai-compat"` only and that `command` is ignored for that kind. Update the `kind` comment:
   `claude` and `openai-compat` are IMPLEMENTED.
   **This block is drift-tested byte-for-byte** against the plan-breakdown schemas reference by the
   `canonical-schema:promptRunners` sentinel - task 25 mirrors it, so get it right here first.
