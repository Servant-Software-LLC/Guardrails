# catches: an implementation that leaves one of the seven PromptInvocation sites carrying the wrong role. A Claude-invariance test alone would pass with AiMergeResolver mislabelled Advisory - which is how a write-capable merge would land on a runner with no write tool.
#
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "PromptRoleSeamTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
