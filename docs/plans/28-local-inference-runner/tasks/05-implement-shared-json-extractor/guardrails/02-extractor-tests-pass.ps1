# catches: an extractor that does not honour the last-fenced-block-then-last-object contract, or that fails OPEN on malformed input instead of yielding nothing.
#
...
#          Verified discriminating: 'PromptJsonExtractorTests' is a substring of no other test class in this plan or in
#          the target project.
...
$filter = 'FullyQualifiedName~PromptJsonExtractorTests'
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "PromptJsonExtractorTests is not green. Fix the implementation - do NOT edit the tests (they are outside this task's writeScope and an edit fails the write-scope check)."
    exit 1
}
...
Write-Output "PromptJsonExtractorTests green: $executed executed, 0 failed."
exit 0
