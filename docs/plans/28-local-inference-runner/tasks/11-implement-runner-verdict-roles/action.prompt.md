...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "11-implement-runner-verdict-roles": { "someKey": "someValue" } }`.
...
- **Verdict transcription (section 6.4).** The runner writes the verdict file, and the rule that makes
  that safe is that it may only ever **TRANSCRIBE**. Extract with `PromptJsonExtractor` (task 04);
...
- **Registry construction.** `PromptRunnerRegistry` constructs an `OpenAiCompatPromptRunner` for
  ...
  pins `Implemented` against the dispatch switch must still pass.
- **`--settings` is fatal.** If an `ExtraArg` of `--settings` reaches this runner, THROW rather than
  proceeding - after task 13's splice condition it is genuinely unreachable, so its arrival means the
