...
#          business touching. Scoped to the existing prompt-runner suite, which this task edits shared code in - not the whole
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "This task broke the existing prompt-runner suite ($failed failed of $executed). Its own tests passing is not enough - fix the regression."
    exit 1
}
...
Write-Output "Neighbours intact: $executed executed in the existing prompt-runner suite, 0 failed."
exit 0
