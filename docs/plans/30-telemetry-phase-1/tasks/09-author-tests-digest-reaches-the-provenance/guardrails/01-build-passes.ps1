...
#          these tests reference members declared by TWO upstream tasks (AttemptProvenance.ModelDigest
#          from 03, PromptResult/ActionRun.ModelDigest from 04). A file written against a member that
#          never landed fails at COMPILE time, and that must be diagnosed as an upstream shape gap
#          rather than reported as four unbound behaviours.
#
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "tests/Guardrails.Core.Tests does not build. The tests must COMPILE and FAIL - not compiling is a mistake to fix, not the intended TDD red. Check that AttemptProvenance.ModelDigest (task 03) and PromptResult.ModelDigest (task 04) both exist, and that the stub IPromptRunner and every fixture helper live INSIDE this task's one test file rather than in a shared helper outside the writeScope."
    exit 1
}

Write-Output "Core test project builds - the digest-provenance tests compile."
exit 0
