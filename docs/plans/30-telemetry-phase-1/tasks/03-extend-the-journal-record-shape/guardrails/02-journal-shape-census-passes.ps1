# catches: a Phase-1 journal member that is DECLARED but not proven - and, distinctly, a pinned
#          behaviour that quietly stopped existing. A suite-level green exit certifies only that the
#          tests which RAN passed; a test deleted, renamed, or [Skip]ped leaves the suite green and the
#          behaviour unobserved, which is exactly how the JsonIgnore assertion (the one defect no
#          property-exists test can see) would go missing. One entry per enumerated behaviour, each
#          observed 'Passed' in the runner's OWN TRX - never merely discovered by name.
#
...
#          pair's, because nothing throws and a hollow test still passes. It is carried instead by the
#          prompt's requirement that every 'omitted when null' assertion serialize through the REAL
#          JournalJson.Options and inspect the emitted JSON - a hollow body cannot fake a round-trip
#          through the real serializer - and by the positive control each one is paired with.
#
# NO EXEMPTIONS. All seven rows demand 'Passed'.
#
...
# shape 3, the class term alone. 'Phase1JournalShapeTests' was checked against all 194 Core test class
...
$filter = 'FullyQualifiedName~Phase1JournalShapeTests'

# THE MANIFEST: each enumerated behaviour -> the test method name the ACTION PROMPT PINNED for it.
$manifest = [ordered]@{
    'Bucket rides TaskJournalEntry and is omitted when null'        = 'BucketRidesTheTaskEntry_AndIsOmittedWhenNull'
    'ModelDigest rides AttemptProvenance and is omitted when null'  = 'ModelDigestRidesTheProvenance_AndIsOmittedWhenNull'
    'RouteWarm rides AttemptProvenance and is omitted when null'    = 'RouteWarmRidesTheProvenance_AndIsOmittedWhenNull'
    'Turns ride AttemptRecord and are omitted when null'            = 'TurnsRideTheAttemptRecord_AndAreOmittedWhenNull'
    'Segments ride AttemptRecord and are omitted when null'         = 'SegmentsRideTheAttemptRecord_AndAreOmittedWhenNull'
    'RunEnvironment rides JournalDocument and is omitted when null' = 'RunEnvironmentRidesTheDocument_AndIsOmittedWhenNull'
    'every Phase-1 member round-trips through JournalJson'          = 'EveryPhase1MemberRoundTripsThroughJournalJson'
}

$resultsDir = Join-Path ([System.IO.Path]::GetTempPath()) "guardrails-journal-shape-census-$PID"
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "Phase1JournalShapeTests is red (or the test host failed to start). This pair is collapsed: the record declaration in src/Guardrails.Core/Journal/JournalModel.cs IS the implementation, so a failing test here means the declaration and the test disagree - fix whichever is wrong, both are in this task's writeScope. If an 'omitted when null' assertion is the failure, the member is missing its [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]: JournalJson sets DefaultIgnoreCondition = Never, so without the attribute the key is written as null."
    exit 1
}
...
if ($executed -lt 1) {
    Write-Output "FILTER MATCHED NOTHING: 0 tests executed for '$filter'. The class Phase1JournalShapeTests was not found, or the filter is malformed - this guardrail is certifying nothing. This is NOT a finding about the journal records."
    exit 1
}
...
Write-Output "Per-test census: all $($manifest.Count) enumerated journal-shape behaviours are bound to a pinned test observed Passed ($executed test(s) executed)."
exit 0
