# catches: an implementation whose behaviour deviates from the ten behaviours THIS task pair owns -
#          most sharply, the three the plan's table does not spell out: conflating a NULL writeScope
#          with an empty one (the off-switch is not the "writes nothing" declaration), letting a
#          TDD-red guardrail pull a src+tests task out of code+tests, and forcing an unmatched write
#          surface into the nearest bucket instead of returning null. It also catches a widened
#          signature, because ClassifySignatureAdmitsNoTaskIdentity reads the parameter list by
#          reflection and must stay green here.
#
...
#          'TaskFingerprintBucketTests' was checked against all 195 existing Core test class names and
...
if ($code -ne 0) {
    ...
    Write-Output ""
    Write-Output "TaskFingerprintBucketTests is red. Fix src/Guardrails.Core/Telemetry/TaskFingerprintBucket.cs - do NOT edit the test file, which is outside this task's writeScope and would fail the write-scope check. If ClassifySignatureAdmitsNoTaskIdentity is the failure, you changed the signature: revert it."
    exit 1
}
...
if ($executed -lt 1) {
    Write-Output "FILTER MATCHED NOTHING: 0 tests executed for '$filter'. The class TaskFingerprintBucketTests was not found, or the filter is malformed - this guardrail is certifying nothing. This is NOT a finding about the implementation."
    exit 1
}

Write-Output "TaskFingerprintBucketTests green: $executed tests executed, 0 failed."
exit 0
