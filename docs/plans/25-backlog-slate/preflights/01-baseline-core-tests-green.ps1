...
#          the area this plan's four deliverables all add tests into, are already failing on the
#          starting code. Asserting them green BEFORE the DAG means a later task's tests-pass failure
#          is attributable to THAT task rather than to pre-existing breakage, and the new tests' TDD
#          red is unambiguous (#181). Re-emits the failure DETAIL at the END so a red baseline's WHY
#          reaches the halt feedback, not just `[FAIL] <name>` (#179).
#
# Scope: the AREA (this one test project), never the whole suite - a whole-solution test run hits the
# #165/#176 compile-coupling trap on a mid-TDD tree and dead-ends a run no work task can rescue. All
# six test files this plan authors land in tests/Guardrails.Core.Tests (Samples/, Providers/, LogSite/,
# Graph/, ModelTiering/), so one area covers the plan and the per-area dedupe yields exactly this file.
#
...
# this plan conjoins its OWN test class beside it (task 05/06 conjoin ~BarrierWaitTests).
#
...
#   FullyQualifiedName~TransientBackoffTests                            -> Passed: 4
#   FullyQualifiedName~TransientBackoffTests&Category!=BacklogSlate     -> Passed: 4
#     => `!=` INCLUDES tests that carry no Category trait at all. Every existing test in this project
#        is one of those (grep for "BacklogSlate" over tests/ and src/ returns 0 hits on the untouched
#        tree), so this filter does NOT vacuously select zero and red-halt before the DAG.
...
#        preflight will genuinely skip them rather than merely appearing to - and a zero match exits 0,
#        which is why the executed-count guard below is not optional.
#
