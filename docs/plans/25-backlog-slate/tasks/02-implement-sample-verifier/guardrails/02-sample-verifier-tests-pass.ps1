# catches: an implementation whose behavior deviates from the tests THIS task pair owns - a verifier
#          that supplies the sample only one of the two ways the corpus binds it (so half the pairs are
#          silently verified against the REAL tree instead), a reversed-polarity pair reported as two
#          findings instead of one, a stale/orphan sample skipped rather than reported, a prompt-guardrail
#          pair swallowed in silence, or a finding that names no exit code and so cannot be acted on.
...
$filter = 'Category=BacklogSlate&FullyQualifiedName~SampleVerifierTests'   # copied VERBATIM from the pair's inverse half (task 01)
...
if ($testExit -ne 0) {
    ...
    Write-Output "SampleVerifierTests failing - SampleVerifier is not implemented to the spec those tests pin (see failure details above)"
    exit 1
}
...
if ($ran -lt 1) {
    Write-Output "exit 0 but ZERO tests executed - this guardrail certified nothing. The --filter '$filter' matched no tests, is malformed, or every matched test is [Skip]ped. Check it against the tests this task pair actually owns (class SampleVerifierTests, trait Category=BacklogSlate)."
    exit 1
}
exit 0
