# catches: a routing change that does not COMPILE, and a change that compiles in Guardrails.Cli but
#          breaks the test assembly that drives it. Building src/Guardrails.Cli alone would leave
#          ServeDiagramTests.cs uncompiled, so guardrail 02's `--no-build` would then run against a
...
# errors. It is NOT carried onto the dotnet test in 02/03 - there it would delete the failure detail
# the #179 re-emit exists to surface (dotnet.md 4.3).
dotnet build tests/Guardrails.Integration.Tests --nologo -v q
if ($LASTEXITCODE -ne 0) {
    Write-Output "tests/Guardrails.Integration.Tests does not build - the LogServer routing change (or a LogSiteRenderer / OnTheFlyDiagramObserver edit) is not type-correct (see the compiler errors above)"
    exit 1
}
exit 0
