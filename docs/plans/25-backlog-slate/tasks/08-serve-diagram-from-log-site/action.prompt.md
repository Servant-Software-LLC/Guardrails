...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "08-serve-diagram-from-log-site": { "someKey": "someValue" } }`. The harness
...
resolve. Turn the tests task 07 authored green — **without changing them**.
...
scope and you may not edit it. Your guardrail runs the SAME filter that task 07's red census ran.
...
- **`LogServer._logsRoot`** is the `logs/<runId>/` tree the run writes into;
  ...
  needs no change at all. Verify before you touch it; the observer is also wanted by task 11's
