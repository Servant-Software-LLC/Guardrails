...
#          task-level --filter selected. This plan edits Scheduler.cs, HtmlDiagramRenderer.cs and the
#          preflight phase, all of which have substantial existing coverage OUTSIDE the BacklogSlate
#          trait (SchedulerTests, SchedulerWaveExecutionTests, HtmlDiagramRendererTests, ...), so the
#          whole-area suite is the only check that can see that. It is also the second half of the #181
#          baseline: green START before the DAG (preflights/01), green END on the merged HEAD (here).
...
# downstream TDD task has not run yet - so the deliberately-red author-tests of one half (tasks 01, 05,
# 07, 10 all author intentionally-failing tests) would red-halt a correct run. That is the #125
