...
if ($buildExit -ne 0) {
    Write-Output "tests/Guardrails.Core.Tests does not compile - GuardrailAbortTests.cs has a build error (see above). A non-compiling test file is NOT a TDD red."
    exit 1
}
exit 0
