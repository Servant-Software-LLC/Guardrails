...
:::question
{"id": "d21-milestone-scope", "title": "Approve all three milestones, or ship M1 (deterministic capture, no AI, no spend) and make M2/M3 earn their place?", "mode": "single", "options": ["All three — M1, M2 and M3 together", "M1 only for now; revisit M2/M3 against real aborts", "M1 + M2 (the judge), holding M3 (the draft) back"], "recommended": "All three — M1, M2 and M3 together", "rationale": "The design makes this cleanly separable on purpose: M1 is the evidence #451 lacked, it involves no AI and no spend, and section 8 says it 'stands even if the review rejects the AI half'. So option 2 is genuinely available and costs you nothing you cannot add later. I still recommend all three, and the reason is DA3 rather than optimism: the author concedes that 'unclassified will be the modal answer' is plausible and NOT disprovable in advance. That is a bet you can only settle by running it, and section 8 already writes down the numbers that would settle it (fewer than half of the first 10 eligible aborts producing an agreed classification at moderate confidence). Shipping M1 alone defers the bet without gathering the evidence that resolves it, so you would be back here with the same open question and no more data. Note the floor is real either way: even at 100% unclassified, M1 is a strict improvement and the ruledOut requirement means an unclassified verdict still narrows. Cost is deliberately not an argument here.", "target": "human"}
:::

:::question
{"id": "d21-falsification-owner", "title": "Section 8's falsification criteria are good. WHO checks them, and when?", "mode": "single", "options": ["The harness measures it: count distinct untyped abort signatures and surface the trend", "A scheduled human review after the first 10 triage-eligible aborts", "Leave it as documented intent; revisit if it feels wrong"], "recommended": "The harness measures it: count distinct untyped abort signatures and surface the trend", "rationale": "This is my main criticism of an otherwise unusually complete design, and it is a shape this repo has been bitten by before. Section 8 states four falsification criteria in real numbers — the strongest being DA1's, that if the count of DISTINCT UNTYPED ABORT SIGNATURES is flat or rising then triage has become a substitute for typing faults and should be cut. That is exactly the right test. But nothing in the design schedules anyone to run it, and a falsification criterion nobody measures is decoration: it reads as rigour while doing the work of nothing. The self-liquidating property is the whole answer to DA1 — the strongest objection the design faces — so it is the one claim that most needs to be observed rather than asserted. The slug is already harness-computed (section 4.3), so counting distinct signatures across runs is nearly free, and section 8 already names cross-run recurrence as a cheap later addition. Option 3 is honest about what usually happens and is fine if you would rather not build the instrument, but then the DA1 concession should be stated as an intention rather than as a control.", "target": "human"}
:::

:::question
{"id": "d21-triage-default", "title": "Should triage run by default, given DA4 — it fires while the operator is waiting at a dead run?", "mode": "single", "options": ["On by default, with --no-fault-triage as the off switch (as designed)", "Opt-in for v1; make it default once the latency is measured", "On by default only in autonomous/unattended runs, opt-in when attended"], "recommended": "On by default only in autonomous/unattended runs, opt-in when attended", "rationale": "I am departing from the design here, and the reason is that DA4's answer depends on something the design treats as one case. Unattended is where this feature is worth most and costs least: nobody is watching, the run is already dead, and a report waiting in the morning is pure gain. Attended is the opposite — the operator is sitting at a dead run, and section 8 already anticipates 'operators report interrupting the triage wait' as falsifying evidence. Splitting on attendedness gets the value where the wait is free and avoids buying the exact complaint the design predicts. Against my own recommendation: it adds a mode distinction to a feature whose virtue is being strictly additive, and section 5.1's ordering already means the operator has today's full information BEFORE the wait begins, with the wait announced — which is a real mitigation and may be enough. The design's own position is option 1 and it is defensible; option 2 is the cautious read and I would argue against it, because an opt-in post-mortem is one nobody remembers to opt into at the moment it would have helped.", "target": "human"}
:::
