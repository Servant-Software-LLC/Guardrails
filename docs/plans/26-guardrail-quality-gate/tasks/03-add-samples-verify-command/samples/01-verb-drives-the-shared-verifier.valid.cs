...
/// <summary>
/// Representative CORRECT shape: <c>guardrails samples verify [folder]</c> drives the SHARED
/// <see cref="SampleVerifier"/> — the same type the pre-DAG preflight phase runs — so the verb and the
/// phase can never disagree about whether a committed pair is sound.
/// </summary>
public static class SamplesCommand
{
    ...
    private static async Task<int> RunAsync(string folder, IConsoleIo io, CancellationToken cancellationToken)
    {
        ...
        // validate. A folder that loads but carries diagnostics is still sample-verifiable.
        ...
        SampleVerifyResult result = await SampleVerifier.VerifyAsync(
            plan, new ProcessRunner(), TimeSpan.FromSeconds(60), cancellationToken);

        foreach (SampleFinding finding in result.Findings)
        {
            io.Out.WriteLine(
                $"{finding.Kind}: {finding.SamplePath} against {finding.GuardrailPath} → exit {finding.ObservedExitCode?.ToString() ?? "(none)"}");
