...
///
/// Delivered from task 04 this file is unrepairable downstream: task 05's writeScope is
/// PlanPreflightPhase.cs only, so the same clauses at task 05 would red every attempt with no in-scope
/// remedy. That is why the ban is enforced HERE, where the file is owned.
/// </summary>
...
public sealed class SampleVerifierWiringTests : IDisposable
{
    ...
    {
        string planDir = CreatePlan("journalled", soundPair: false);
        PlanDefinition plan = LoadPlan(planDir);

        SampleVerifyResult result = await SampleVerifier.VerifyAsync(
            plan, new ProcessRunner(), TimeSpan.FromSeconds(30), CancellationToken.None);

        Assert.Contains("01-subject-check", string.Join("\n", result.Findings.Select(f => f.Message)),
    ...
    {
        string planDir = CreatePlan("cli", soundPair: false);
        PlanDefinition plan = LoadPlan(planDir);
    ...
    private string CreatePlan(string name, bool soundPair)
    {
        ...
        File.WriteAllText(Path.Combine(taskDir, "action" + ext), "exit 0\n");
