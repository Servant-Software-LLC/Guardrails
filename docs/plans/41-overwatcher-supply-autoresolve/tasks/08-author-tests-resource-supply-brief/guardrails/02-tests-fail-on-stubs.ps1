...
try {
    ...
    foreach ($behaviour in $manifest.Keys) {
        ...
        if ($expect -eq 'Executed') {
            ...
            # this census. TheGenericDiagnoseBriefIsUnchanged is the never-weaker floor over SHIPPED code
            # this task does not touch: if it is Failed, the stub tree is broken in a way that invalidates
            # every red beside it, and that must red HERE rather than bill task 09.
