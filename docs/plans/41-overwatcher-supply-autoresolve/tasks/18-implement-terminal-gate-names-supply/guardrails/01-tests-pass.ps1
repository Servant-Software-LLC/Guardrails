# catches: an implementation that does not satisfy the tests task 17 authored - and, via the zero-match
...
# SAME string as task 17's inverse half - copied verbatim, so the two halves of the pair cannot drift.
...
if ($code -ne 0) {
    ...
    # the Expected/Actual payload of an Assert.Equal failure - and the never-weaker row here is a
    # byte-equality assertion, where that payload IS the finding (it shows exactly what was appended to a
    # halt that should have been left alone).
    $inBlock = $false
    $emitted = 0
    ...
    Write-Output ""
    Write-Output "SuppliedTerminalGateHaltTests failing - the terminal gate halt does not disclose supplied/refreshed content, or it changed a halt that carries none (see failure details above)."
    exit 1
}
...
Write-Output "SuppliedTerminalGateHaltTests green: $ran test(s) executed, all passing."
exit 0
