...
#          red census below cannot tell garbage from a real red - and the implementation task (task 16,
#          whose writeScope EXCLUDES the test file) could not fix the compile error anyway, dead-ending
#          the run (#155).
#          Subject: tests/Guardrails.Integration.Tests/Supply/SuppliedHaltTextTests.cs
#          The WHOLE solution is built, not just the test project: this file drives RunCommand's public
#          render seams, and the Cli assembly ships no InternalsVisibleTo, so a signature it relies on
#          breaking elsewhere must surface here rather than at the terminal gate.
