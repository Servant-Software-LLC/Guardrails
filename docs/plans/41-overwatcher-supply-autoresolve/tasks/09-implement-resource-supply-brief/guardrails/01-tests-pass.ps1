...
# The SAME $filter string as this pair's inverse half (tasks/08-.../guardrails/02-tests-fail-on-stubs.ps1),
# copied verbatim so the two halves of the TDD pair can never drift apart.
...
if ($code -ne 0) {
    ...
    # and `System.NotImplementedException : ...` - the dominant first-attempt failure here - leaving
    # `Error Message:` followed by nothing. The brief rows also fail with Assert.Equal string diffs,
    # whose `Expected:`/`Actual:` payload sits at column 0 and is most of the diagnosis.
    $inBlock = $false
    $emitted = 0
    ...
    Write-Output ""
    Write-Output "The missing-resource brief rows are failing - the brief or the trigger token is not implemented to spec (see failure details above). If ONLY the real-seam row fails, the brief TEXT is fine and the PromptInvocation is wrong: read that failure before editing brief text."
    exit 1
}
