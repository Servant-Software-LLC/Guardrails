...
try {
    ...
    if ($nodes.Count -lt 1) {
        ...
        Write-Output "PRECONDITION: the filter 'FullyQualifiedName~WaveDeliveryDiagnosticsTests' matched NO tests. A zero-match filter exits 0 and certifies nothing."
        exit 1
    }
