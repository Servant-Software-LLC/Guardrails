...
on `LastMergeOnSuccessDetail`. Construct the real provider over a temp repo and make the delivery refuse —
move the branch underneath it, or commit a conflicting change on the user's branch.
...
The other seven tests MUST COMPILE and FAIL. Do NOT implement the halt.
