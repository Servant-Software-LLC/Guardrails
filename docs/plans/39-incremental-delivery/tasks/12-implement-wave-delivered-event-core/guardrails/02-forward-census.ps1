# catches: a DECLARED-EXEMPT census row quietly ceasing to exist or to pass. Task 11's red census
#          excuses ADecoratorThatDropsTheEvent_IsCaught from being Failed (the negative control is
#          green on arrival) but NOT from existing; this is the other half of that bargain — the pinned
...
try {
    ...
    & dotnet test "tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj" -c Debug --nologo `
        --filter "FullyQualifiedName~WaveDeliveredEventTests" `
    ...
    if ($results_nodes.Count -lt 1) {
        ...
        Write-Output "PRECONDITION: the filter 'FullyQualifiedName~WaveDeliveredEventTests' matched NO tests. A zero-match filter exits 0 and certifies nothing — fix the filter or the class name."
        exit 1
    }
