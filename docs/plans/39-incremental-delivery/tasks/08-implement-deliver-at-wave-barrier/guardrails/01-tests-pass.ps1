...
#
#          ALSO runs MergeOnSuccessTests, RunOutcomeWiringTests and DivergenceDeliveryGateTests (review
#          2026-09-13): this task EXTRACTS Finalize's delivery decision so the barrier can share it, and
#          those three classes pin the run-end behavior that must not change - the #340 opt-out, the #361
#          interlock and its #597 override, the #588/#448/#149 refusals, and the #556 rule that a run whose
#          task definition diverged mid-run delivers nothing. Without them a broken Finalize reaches only the
...
$out = & dotnet test "tests/Guardrails.Integration.Tests/Guardrails.Integration.Tests.csproj" -c Debug --nologo --filter "FullyQualifiedName~WaveBarrierDeliveryTests|FullyQualifiedName~MergeOnSuccessTests|FullyQualifiedName~RunOutcomeWiringTests|FullyQualifiedName~DivergenceDeliveryGateTests" 2>&1 | Out-String
$code = $LASTEXITCODE
...
if (($passed + $failed) -lt 1) {
    Write-Output "ZERO-MATCH: the filter 'FullyQualifiedName~WaveBarrierDeliveryTests|FullyQualifiedName~MergeOnSuccessTests|FullyQualifiedName~RunOutcomeWiringTests|FullyQualifiedName~DivergenceDeliveryGateTests' executed no tests — that exits 0 and would certify this task on an empty set."
    exit 1
}
