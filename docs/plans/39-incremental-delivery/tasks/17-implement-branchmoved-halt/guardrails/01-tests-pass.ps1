...
#            - Guardrails.Integration.Tests: this task's own BranchMovedHaltTests, plus task 07's
#              WaveBarrierDeliveryTests. This task adds the refused-delivery halt beside task 08's
#              trial-gate halt (round 5, d39-trial-gate-failure). A refusal check that swallowed a failed
#              trial-tree gate, turning its ExitGateFailed halt into DeliveryRefused with no `halt` section,
#              used to pass every guardrail on this task.
...
$runs = @(
    @{ Project = 'tests/Guardrails.Integration.Tests/Guardrails.Integration.Tests.csproj'; Classes = @('BranchMovedHaltTests', 'WaveBarrierDeliveryTests') },
