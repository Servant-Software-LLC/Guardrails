...
$out = & dotnet test "tests/Guardrails.Integration.Tests/Guardrails.Integration.Tests.csproj" -c Debug --nologo --filter "FullyQualifiedName~PostDeliveryRefreshTests" 2>&1 | Out-String
$code = $LASTEXITCODE
...
if (($passed + $failed) -lt 1) {
    Write-Output "ZERO-MATCH: the filter 'FullyQualifiedName~PostDeliveryRefreshTests' executed no tests — that exits 0 and would certify this task on an empty set."
    exit 1
}
