...
$pinned = @(
    ...
    'GR2078_IsSilentForAWaveThatFollowsNoDelivery',
    'GR2079_FiresWhenADeliveringWaveHasNoExitGate',
    'GR2079_IsSilentWhenTheWaveHasAnExitGate',
    'BothAreWarnings_AndDoNotMoveTheExitCode',
...
try {
    ...
    & dotnet test "tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj" -c Debug --nologo `
        --filter "FullyQualifiedName~WaveDeliveryDiagnosticsTests" `
    ...
    if ($results_nodes.Count -lt 1) {
        ...
        Write-Output "PRECONDITION: the filter 'FullyQualifiedName~WaveDeliveryDiagnosticsTests' matched NO tests. A zero-match filter exits 0 and certifies nothing — fix the filter or the class name."
        exit 1
    }
