# catches: starting from RED - the pre-existing RetryPolicyTests are already failing before this plan
#          touches the RetryPolicy code they cover, so a later task's tests-pass failure would be
#          misattributed to that task.
$log = dotnet test tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj -c Release --filter "FullyQualifiedName~RetryPolicyTests" 2>&1 | Out-String
$code = $LASTEXITCODE
Write-Output $log
...
if ($failedCount -eq 0) {
    ...
    Write-Output "$failedCount existing RetryPolicyTests test(s) are ALREADY failing on the starting code - fix that pre-existing breakage before this plan builds on it"
}
exit 1
