...
- Write everything you publish under your task's WAVE-QUALIFIED id as the single
  ...
  `{ "wave-01-correct-the-record/03-correct-plan-breakdown-allowlist-wording": { "someKey": "someValue" } }`. The harness REJECTS a fragment keyed by
...
## Task
The #252 guidance in **SKILL.md, references/schemas.md and references/example-breakdown.md** under `.claude/skills/plan-breakdown/` states that state-mutating git commands
...
**Scope boundary (harness-enforced):** Write only under `.claude/skills/plan-breakdown/`. After this task completes the
