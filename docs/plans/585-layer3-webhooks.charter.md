...
:::question
{"id": "detail-default", "title": "Withhold `detail` from webhook deliveries by default?", "mode": "single", "options": ["Withhold by default; `--on-event-detail` opts in", "Send `detail` by default; add a flag to suppress it", "Never send `detail` at all, with no opt-in"], "recommended": "Withhold by default; `--on-event-detail` opts in", "rationale": "#585's own framing said `faultKind` is 'the one value on the row that can carry an absolute path, a token, or a fragment of source', and that framing is now out of date. `detail` is strictly worse: it appears on two of six kinds, is uncapped, and #179 DELIBERATELY made generated test guardrails re-emit assertions and stack traces into it, while `task-settled`'s splices absolute feedback.md paths and an agent's raw needs-human question. Applying the bar you already set for `faultKind` consistently produces this default. The honest counter-argument, which the design records: the needs-human QUESTION has no carrier other than `detail`, so withholding it means a webhook consumer learns a run needs a human but not what it was asked.", "target": "human", "answer": ["Withhold by default; \u0060--on-event-detail\u0060 opts in"]}
:::

:::question
{"id": "detail-optin-flag", "title": "Is `--on-event-detail` worth shipping, or is it the cheapest thing to cut?", "mode": "single", "options": ["Ship it — the opt-in is the point of a default", "Cut it — withhold `detail` unconditionally in v1"], "recommended": "Ship it — the opt-in is the point of a default", "rationale": "The design's author names this as the cheapest remaining thing to cut, so it is put to you rather than assumed. Keeping it means a second flag, a second code path and a second thing to document, for a field most consumers should not receive. Cutting it means an operator running a private endpoint on their own machine cannot get the guardrail failure reason without reading events.jsonl — which is exactly the filesystem read #585 was filed to remove, reintroduced for the one field that most needs a reason attached.", "target": "human", "answer": ["Ship it \u2014 the opt-in is the point of a default"]}
:::
...
:::question
{"id": "untracked-deferrals", "title": "Two deferrals in §8 say 'New issue' but name none — file them now?", "mode": "single", "options": ["File both now (HMAC signature, multiple endpoints)", "File only the HMAC signature one", "Leave both untracked — §8 recording them is enough"], "recommended": "File both now (HMAC signature, multiple endpoints)", "rationale": "Raised because Charter only checks that a deferral names SOMETHING, and verifying is the agent's job. §8 defers an HMAC body signature and multiple endpoints, each marked 'New issue', but neither names one — so once this design is archived those two rows are the only record, and nobody re-reads an archived design. Both have real content already worked out: the HMAC row names what it needs (a documented canonicalization and verification recipe), and the multi-endpoint row names its blocker (one GUARDRAILS_ON_EVENT_AUTH across two endpoints sends B's token to A). Filing is a decision with an owner attached, so it is yours rather than something I do quietly.", "target": "human", "answer": ["File both now (HMAC signature, multiple endpoints)"]}
:::
