...
**Q: Add a `bracket` field to the §8.1 row shape, one week after #595 settled it?** — Answered: Add `bracket` to the row (design's proposal)
_Question — id: `bracket-field`; mode: `single`; target: `human`; options: `Add `bracket` to the row (design's proposal)`, `Delivery-only header instead — keep the row shape frozen`, `Neither — accept that a resumed run is discarded, and document it`; recommended: `Add `bracket` to the row (design's proposal)`_
_Why: This is the one place the design changes a contract PR #599 settled a week ago, so it deserves a deliberate yes. The bug it fixes is silent: `seq` restarts at 1 per process, so a resumed run re-POSTs `seq: 1` under the same `runId`, and a receiver deduplicating on `(runId, seq)` — which is exactly what §8.1 currently tells it the key is — discards the entire resumed run with nothing anywhere reporting it. A file reader survives this positionally; a webhook receiver cannot. The change is additive and no existing test asserts exact row JSON. The rejected alternative, a delivery-only header, keeps the row frozen but splits the dedupe key across the body and the transport, so a consumer reading events.jsonl after a dropped delivery cannot reconstruct it._
...
**Q: Should the circuit breaker be able to re-close, so a recovered endpoint still gets `run-finished`?** — Answered: Always attempt `run-finished` even when the circuit is open
_Question — id: `circuit-breaker`; mode: `single`; target: `human`; options: `Leave it latched open (design as written)`, `Let it re-close after a cooldown`, `Always attempt `run-finished` even when the circuit is open`; recommended: `Always attempt `run-finished` even when the circuit is open`_
_Why: The design's author flags this as the thing they hold least firmly. As written the circuit opens after 5 consecutive failed rows and never re-closes — which means it can drop `run-finished`, the single row a CI wrapper exists to receive, because of failures that happened minutes earlier and may already have healed. The design says the fix is about ten lines. I lean to the third option rather than a cooldown because it is the smallest change that protects the one row whose loss actually costs something, without reintroducing a timer whose tuning nobody can justify. Worth noting the interaction with #603: on Ctrl-C the process budget is ~2s, so a terminal attempt has to fit inside that whatever we choose here._
...
**Q: Withhold `detail` from webhook deliveries by default?** — Answered: Withhold by default; `--on-event-detail` opts in
_Question — id: `detail-default`; mode: `single`; target: `human`; options: `Withhold by default; `--on-event-detail` opts in`, `Send `detail` by default; add a flag to suppress it`, `Never send `detail` at all, with no opt-in`; recommended: `Withhold by default; `--on-event-detail` opts in`_
_Why: #585's own framing said `faultKind` is 'the one value on the row that can carry an absolute path, a token, or a fragment of source', and that framing is now out of date. `detail` is strictly worse: it appears on two of six kinds, is uncapped, and #179 DELIBERATELY made generated test guardrails re-emit assertions and stack traces into it, while `task-settled`'s splices absolute feedback.md paths and an agent's raw needs-human question. Applying the bar you already set for `faultKind` consistently produces this default. The honest counter-argument, which the design records: the needs-human QUESTION has no carrier other than `detail`, so withholding it means a webhook consumer learns a run needs a human but not what it was asked._

**Q: Is `--on-event-detail` worth shipping, or is it the cheapest thing to cut?** — Answered: Ship it — the opt-in is the point of a default
_Question — id: `detail-optin-flag`; mode: `single`; target: `human`; options: `Ship it — the opt-in is the point of a default`, `Cut it — withhold `detail` unconditionally in v1`; recommended: `Ship it — the opt-in is the point of a default`_
_Why: The design's author names this as the cheapest remaining thing to cut, so it is put to you rather than assumed. Keeping it means a second flag, a second code path and a second thing to document, for a field most consumers should not receive. Cutting it means an operator running a private endpoint on their own machine cannot get the guardrail failure reason without reading events.jsonl — which is exactly the filesystem read #585 was filed to remove, reintroduced for the one field that most needs a reason attached._
...
**Q: Two deferrals in §8 say 'New issue' but name none — file them now?** — Answered: File both now (HMAC signature, multiple endpoints)
_Question — id: `untracked-deferrals`; mode: `single`; target: `human`; options: `File both now (HMAC signature, multiple endpoints)`, `File only the HMAC signature one`, `Leave both untracked — §8 recording them is enough`; recommended: `File both now (HMAC signature, multiple endpoints)`_
_Why: Raised because Charter only checks that a deferral names SOMETHING, and verifying is the agent's job. §8 defers an HMAC body signature and multiple endpoints, each marked 'New issue', but neither names one — so once this design is archived those two rows are the only record, and nobody re-reads an archived design. Both have real content already worked out: the HMAC row names what it needs (a documented canonicalization and verification recipe), and the multi-endpoint row names its blocker (one GUARDRAILS_ON_EVENT_AUTH across two endpoints sends B's token to A). Filing is a decision with an owner attached, so it is yours rather than something I do quietly._
...
<!-- charter: plan-sha256=54fcd5dccbe1e7fee6912a5da9e376d9b48c837eb0e61d2adaa53e5488b859ef -->