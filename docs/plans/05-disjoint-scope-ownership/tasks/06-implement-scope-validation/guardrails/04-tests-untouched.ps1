# catches: "making the tests pass" by editing the M3 validation test file instead of the
...
$storedHashes = $state.'05-author-scope-validation-tests'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '05-author-scope-validation-tests.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
