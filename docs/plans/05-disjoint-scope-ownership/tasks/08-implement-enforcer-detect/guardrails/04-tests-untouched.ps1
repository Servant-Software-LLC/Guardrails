# catches: "making the tests pass" by editing the M4 enforcer test file instead of the
...
$storedHashes = $state.'07-author-enforcer-detect-tests'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '07-author-enforcer-detect-tests.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
