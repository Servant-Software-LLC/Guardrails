...
$filter = 'Category=PlanSourceProvenance&FullyQualifiedName~PlanSourceRecordTests'   # SAME string as the pair's forward half (task 02)

# THE MANIFEST: each enumerated behaviour -> the test method name the ACTION PROMPT PINNED for it.
# Cross-checked by hand against tasks/01-author-tests-plan-source-record/action.prompt.md - the
# prompt<->manifest agreement is NOT mechanically enforced (measured: validate exits 0 either way).
$manifest = [ordered]@{
    'byte-exact hash comes from the BYTES (a BOM changes it)'   = 'SourceSha256_IsComputedOverRawBytes_NotDecodedText'
    'LF-normalized hash is stable across CRLF / CR / LF'        = 'SourceSha256Lf_IsStableAcrossCrlfAndLf'
    'every charter: comment lands in an OPEN stamps map'        = 'Stamps_CapturesEveryCharterCommentAsAnOpenMap'
    'no charter: comment yields an EMPTY map, never null'       = 'Stamps_IsEmptyWhenThePlanCarriesNoCharterComment'
    'duplicate stamp key: FIRST wins, duplicate reported'       = 'Stamps_FirstWinsOnADuplicateKey'
    'the DECISIONS DELEGATED TO YOU count line is read'         = 'DeclaredDelegatedDecisions_ReadsTheCountLine'
    'no count line yields 0 (absence is unambiguous)'           = 'DeclaredDelegatedDecisions_IsZeroWhenNoCountLineIsPresent'
    'SourceBytes equals the byte length actually read'          = 'SourceBytes_MatchesTheFileLength'
    'PlanHash unchanged by state/plan-source.json'              = 'PlanHash_IsUnchanged_WhenPlanSourceJsonIsPresent'
    'PlanDefinitionHash unchanged by state/plan-source.json'    = 'PlanDefinitionHash_IsUnchanged_WhenPlanSourceJsonIsPresent'
}
