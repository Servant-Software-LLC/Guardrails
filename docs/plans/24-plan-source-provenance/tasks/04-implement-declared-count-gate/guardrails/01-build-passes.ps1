...
if ($LASTEXITCODE -ne 0) {
    Write-Output "tests/Guardrails.Core.Tests does not build - DeclaredCountGate.cs no longer type-checks against DeclaredCountGateTests.cs (the test file is out of this task's write scope: implement to it, do not reshape around it)"
    exit 1
}
exit 0
