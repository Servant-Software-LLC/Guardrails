...
#   InitialBreakdownInvoker\.PrepareInvocation   n/a - file created by this task
#   \bRunReset\s*\.\s*Fresh\b                    n/a - file created by this task
...
if ($code -cnotmatch 'InitialBreakdownInvoker\.PrepareInvocation') {
    $failures += "$f never calls InitialBreakdownInvoker.PrepareInvocation - it does not drive the production entry point, so it cannot prove the recorder is wired into it (a test that builds the record itself passes against an unwired PrepareInvocation)"
}

if ($code -cnotmatch '\bRunReset\s*\.\s*Fresh\b') {
    $failures += "$f never calls RunReset.Fresh - the plan requires a test proving state/plan-source.json SURVIVES --fresh (plan of record section 7), and that survival is the property a later refactor breaks silently"
}
