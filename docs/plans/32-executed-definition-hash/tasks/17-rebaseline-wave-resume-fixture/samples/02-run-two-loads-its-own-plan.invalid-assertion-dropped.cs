...
// .invalid pair - kept committed so a later editor can re-run it by hand): the re-baseline landed AND
// an assertion went with it. Both loads are present, so the deliverable clause is satisfied; only the
// retention count sees that the test now claims less than it shipped claiming.
