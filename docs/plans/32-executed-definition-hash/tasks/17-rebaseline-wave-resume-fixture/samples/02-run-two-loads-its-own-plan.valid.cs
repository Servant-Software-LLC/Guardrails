// A COMPLETE, representative CORRECT artifact for 02-run-two-loads-its-own-plan.ps1 (#468/#302): the
// shipped file with the two-method re-baseline applied. Run 2 in each fixture loads its own plan from
// disk AFTER the edit, exactly as a real resume does. Every assertion, both on-disk fixture writes and
// all fourteen facts are untouched. Complete rather than a fragment - here 'complete' is literally the
// shipped file, which is what makes the .invalid half the real before-state rather than a synthesis.
//
// This header quotes none of the tokens the guardrail keys on (taxonomy 13).
...
public sealed class SchedulerWaveExecutionTests
{
    ...
    public async Task WaveDrift_CompletedWaveChanged_AutoPolicy_RewindsAndReRuns_WithWaveBoundaryDecision()
    {
        ...
        // Loaded AGAIN, after the edit: a resume is a fresh process, so PlanLoader re-reads the folder and
        // the definition each task carries is the edited bytes. Reusing run 1's object would hand run 2
        // nodes pinned to the pre-edit definition, which no real resume ever sees.
        PlanDefinition reloaded = b.Load().Plan!;
        PlanDefinition autoPlan = reloaded with { Config = reloaded.Config with { AutonomyPolicy = AutonomyPolicy.Auto } };
        var e2 = new WaveFakeExecutor();
    ...
    public async Task PendingFutureWaveEdit_IsNotDrift_RunsNormally()
    {
        ...
        // Loaded AGAIN, after the edit - a resume is a fresh process (see the sibling wave-drift test).
        PlanDefinition resumed = b.Load().Plan!;
        var e2 = new WaveFakeExecutor();
        RunJournal j2 = RunJournal.LoadOrCreate(resumed);
        RunReport r2 = await NewScheduler(resumed, e2, j2, new RecordingWorktreeProvider()).RunAsync(resumed, Ct);
