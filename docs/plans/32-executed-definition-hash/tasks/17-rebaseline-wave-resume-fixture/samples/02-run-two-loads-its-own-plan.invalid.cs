// The ONE defect 02-run-two-loads-its-own-plan.ps1 exists to catch, and it is not synthetic: this is
// the SHIPPED file, verbatim. Both wave-resume fixtures model a resume as a second scheduler run over
// the SAME in-memory PlanDefinition, so run 2's TaskNodes carry pins captured BEFORE the on-disk edit
// and the settle-time gate correctly reports a divergence. A real resume is a fresh process that
// re-reads the folder. Committed as the invalid half precisely because the pair is then two-sided
// against reality rather than against something invented for the check.
//
