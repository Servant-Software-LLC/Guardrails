// A COMPLETE, representative CORRECT artifact for 03-file-shape-preserved.ps1 (#468/#302): the
// integration suite as it looks AFTER stage 14. Five facts, none skipped, the rename complete, the
// surviving green-run assertion untouched, and the rationale comment still carrying both markers.
// Kept complete rather than a fragment. This header deliberately quotes NONE of the tokens the
// guardrail keys on - an earlier draft named them and the invalid halves then passed off the header
// alone, a sample that cannot bite (catalogue taxonomy 13).
using System.IO;
...
public sealed class PlanEditedDuringRunTests : IClassFixture<HostRepoCleanlinessGuard>
{
    ...
    public async Task ARunCarryingOnlyAPlanEditObservation_HaltsWithExitTwoAndDoesNotDeliver()
    {
        ...
        Assert.Equal(ExitCodes.NeedsHuman, exit);
        DeliverySection? delivery = ReadDelivery(repo);
        Assert.False(delivery!.Delivered, "a real definition edit blocks delivery; the work stays on the plan branch");
    }
    ...
    public async Task TheRenderedText_CarriesAllThreeSection51Consequences()
    {
        ...
        Assert.Contains("held from load", advisory, StringComparison.OrdinalIgnoreCase);
