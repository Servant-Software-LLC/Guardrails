// The ONE defect 03-gate-uses-the-one-shared-predicate.ps1 exists to catch: a SECOND copy of the
// editor-artifact ignore list, inlined here because it was easier than reaching for the shared
// one. It is behaviourally identical today - it passes every pin in the plan - and it becomes
// wrong on the day someone adds a sixth pattern to one home and not the other.
...
internal sealed partial class Scheduler
{
    ...
    private IReadOnlyList<string> DivergedDefinitionFiles(TaskNode task)
    {
        ...
        foreach ((string label, string absolutePath) in Journal.TaskDefinitionFiles.Enumerate(task))
        {
            ...
            if (fileName is ".DS_Store" or "Thumbs.db"
                || fileName.EndsWith(".swp", StringComparison.OrdinalIgnoreCase)
                || fileName.EndsWith(".orig", StringComparison.OrdinalIgnoreCase)
                || fileName.EndsWith(".rej", StringComparison.OrdinalIgnoreCase))
            {
                continue;
            }
