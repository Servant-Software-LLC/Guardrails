...
// .invalid pair - kept committed so a later editor can re-run it by hand). THE MEASURED HOLE: the
// settle walk is filtered and the LOAD-TIME MAP IS NOT. It looks complete, it passes P16 (whose
// artifact appears mid-run and so is absent from BOTH maps), and it blocks delivery on any artifact
// that was already in the checkout when the plan loaded - which reads as a VANISHED label. One call
// site where there must be two.
