...
// .invalid pair): the coalescing fallback, at wave level. Identical reasoning to the task level -
// for every node the loader built the two branches agree, so it passes every behavioural pin.
...
public static class WaveDefinitionHash
{
    ...
    internal static string ComputeFromPins(WaveNode wave)
    {
        ...
        {
            builder.Append("task:").Append(WaveRelativeId(wave, task)).Append(Unit);
            builder.Append(task.DefinitionHashAtLoad ?? TaskDefinitionHash.Compute(task)).Append(Record);
        }
