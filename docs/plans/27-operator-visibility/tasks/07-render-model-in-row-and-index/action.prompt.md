...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "07-render-model-in-row-and-index": { "someKey": "someValue" } }`. The harness
...
task page. Turn the tests `06-author-tests-model-in-row` authored green — **without changing them**.
...
scope and you may not edit it. Your guardrail runs the SAME filter that `06-author-tests-model-in-row`'s
red census ran. (This plan was RENUMBERED after these prompts were drafted, so task folders are
referred to by NAME throughout — never trust a bare ordinal you find in prose here.)
...
- **`05-raise-attempt-route-resolved`** added `IRunObserver.AttemptRouteResolved(TaskNode task, int attempt, string runner,
...
- **`06-author-tests-model-in-row`** added TWO `throw new NotImplementedException();` stubs to
  `LiveRunObserver` — `ModelCell(string? runner, string? tier, bool climbed, bool substituted,
  bool isScript)` and `ModelCellFromRoute(string runner, string? tier, string? requestedTier)` — and
  authored the tests that drive both.
...
1. **Implement `ModelCell(string? runner, string? tier, bool climbed, bool substituted,
   bool isScript)` and `ModelCellFromRoute(string runner, string? tier, string? requestedTier)`.**
   Both are already unit-tested by `ModelInRowTests`; make those tests pass. The cell
...
2a. **Implement `ModelCellFromRoute(string runner, string? tier, string? requestedTier)` by
   DELEGATING to `ModelCell`** — it computes `climbed: requestedTier is not null` and calls the
   formatter; it does not re-format anything. Its test asserts that AGREEMENT over the whole input
   domain, so an inlined divergent copy fails the moment the two drift.
   **This seam is not ceremony, and a guardrail requires you to CALL it from the route handler.**
   Everything in `LiveRunObserver` except these two static functions is unreachable from a test (the
   constructor starts a process-wide Spectre live region), so the translation the route handler
   performs — and it carries the one rule the event turns on, that `requestedTier`'s PRESENCE is the
   climb signal — would otherwise be checkable only by "is the handler's body non-empty". MEASURED: a
   handler containing one `AnsiConsole.MarkupLine` and a TODO satisfies that, and ships §1.1 verbatim.
   Pulling the translation into a static shrinks what no test can reach to two statements — call it,
   write the result into the cell — and those two are exactly what the guardrail checks.

3. **Populate it from the LAUNCH event, with the post-action event as the CORRECTION.** This is the
   whole change, and the reason `05-raise-attempt-route-resolved` exists:
   - **`AttemptRouteResolved(task, attempt, runner, model, tier, requestedTier)` is the primary
     ...
     attempt starts. Declare the member on `LiveRunObserver` (it is the LEAF observer, and the
     interface member has a **default no-op body**, so simply not declaring it compiles cleanly and
     silently falls back to the post-action event), call
     `ModelCellFromRoute(runner, tier, requestedTier)` from it, and **write the result into the row's
     Model cell** — `_table.UpdateCell(row, 3, new Markup(cell))`, the same shape the file's nine
     existing cell writes use. Computing the cell and discarding it is a measured mutant; so is a
     handler that logs and leaves the cell to `AttemptModelResolved`. Its `requestedTier` is non-null
     ONLY when a §6.2 climb moved the rung — its presence IS the climb signal — so that is what feeds
     `climbed`, via `ModelCellFromRoute`.
