...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `{ "04-replace-meta-refresh": { "someKey": "someValue" } }`. The harness
...
### The tests already exist, they are RED, and you may not touch them

Task 03 — your dependency, already merged — authored
`tests/Guardrails.Core.Tests/Graph/DiagramRefreshTests.cs` and proved, per test, that each of its
four target behaviours **FAILS** against the renderer you are about to change. Those tests are the
specification. **Read that file first: it is the exact statement of what "done" means here**, and
it is more precise than any prose below.

**That file is deliberately OUTSIDE your write scope.** You cannot edit the tests that judge you —
that is the point of the split, not an oversight. If you become convinced one of them is wrong or
incompatible, do **not** change it, weaken it, rename it, or add `[Fact(Skip=...)]`: write
`{"needsHuman": {"question": "<which test, and why it cannot be satisfied>", "kind": "blocked-work"}}`
to the state-out path and stop. An edit to it fails the write-scope check and consumes a retry.

**Files you may write:**

1. `src/Guardrails.Core/Graph/HtmlDiagramRenderer.cs` — the change. This is the real work.
2. `tests/Guardrails.Core.Tests/HtmlDiagramRendererTests.cs` — **only** to retire the one assertion
   your change makes false (see "The three existing tests that block this" below).
3. `tests/Guardrails.Integration.Tests/OnTheFlyDiagramTests.cs` — same, for its one assertion.
4. `tests/Guardrails.Integration.Tests/RunCommandFinalSiteSettleTests.cs` — same, for the ONE
...
**Scope boundary (harness-enforced):** Write only to those four paths. After this task completes,
the harness runs a `git diff` check and rejects any edit outside them — including, specifically,
`tests/Guardrails.Core.Tests/Graph/DiagramRefreshTests.cs` (task 03 owns it), and also
...
### The design, and why it depends on task 02's route

`HtmlDiagramRenderer` is **pure** — it maps strings to a string with no I/O, and that must stay
true. So the page cannot read a status file itself; it has to ask a server. Task 02 (an ancestor
via task 03, already merged) makes the log-site server serve this page at `/diagram.html`, which is
...
The tests task 03 authored and this task's guardrails agree on two literal names. Use them
exactly — a different spelling makes the tests unsatisfiable, and you may not edit them to match.
...
### The tests that judge you, and what each one demands

You do not author these — task 03 did, and its per-test red census already proved every one of the
first four **Failed** against the renderer as you receive it. Read
`tests/Guardrails.Core.Tests/Graph/DiagramRefreshTests.cs` for the exact assertions; this table is a
map, not a substitute.

| Test method name | What makes it go green |
|---|---|
| `DuringRunPage_HasNoMetaRefresh_SoPanZoomAndScrollSurvive` | `Render(..., duringRun: true)` contains **no** `http-equiv` at all. |
| `LivePoll_IsPresentDuringTheRun_AndAbsentOnTheFinalSettledPage` | The during-run page contains `GR_LIVE_POLL_MS`; the `duringRun: false` page does **not**. Both halves, one test — so you cannot satisfy it by removing the poll everywhere. |
| `LivePollInterval_IsAtLeastFiveSeconds_ForADagThatChangesAtTaskBoundaries` | It parses the number out of the during-run page's `GR_LIVE_POLL_MS` assignment and asserts `>= 5000`. It reads the *parsed integer*, so how you format the constant is yours to choose — the value is not. |
| `FileViewFallback_IsPresentAndHidden_SoAnUnpollablePageSaysItIsNotLive` | The during-run page contains the `gr-live-offline` element AND it is hidden in the page's initial state. |
| `SourceSha256AndEmbeddedSource_AreUnchangedByTheLiveUpdateChanges` | **Already green, and it must STAY green.** It pins the `<!-- guardrails:graph v1 source-sha256=... -->` first line and the embedded `id="graph-source"` payload against exactly the kind of chrome change you are making. If you break it you have moved the provenance line or re-encoded the source, and `graph --check` will report every plan in the repo stale. See "Hash neutrality" above. |

**Make them pass by changing `HtmlDiagramRenderer.cs`. That is the only lever you have** — the test
file is outside your write scope, so there is no second way to turn this green.
