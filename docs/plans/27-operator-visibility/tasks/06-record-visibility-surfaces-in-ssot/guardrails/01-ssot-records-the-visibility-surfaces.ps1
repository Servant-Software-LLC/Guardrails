...
#          whole-document refresh and the model surface land in code while the single source of truth
#          still describes a log server that 404s its own diagram, a during-run page that reloads
#          itself every 3 seconds forever, and a run-level index with no model in it. The next agent
#          to touch any of the three reads the SSOT, not the git log, so an unrecorded surface is a
#          surface that gets deleted by a well-meant refactor. Invariant 4: the contract moves in the
...
# The three clauses below are what is left after that subtraction; the residual (that the ambient
