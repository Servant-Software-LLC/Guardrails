...
# tests/Guardrails.Integration.Tests and this one deliberately does not.
# RE-MEASURED 2026-08-29 by /guardrails-review, and the previous number here was WRONG - it is recorded
# rather than quietly corrected, because the way it was wrong is the reusable lesson. It said "THIRTY
# types implement IRunObserver". Thirty is the number of LINES `grep -rn ": IRunObserver"` prints; the
# number of TYPES is NINETEEN. A grep counts lines, and a line is not a declaration. Re-measured with a
# declaration-shaped regex over all 526 .cs files under src/ and tests/ (excluding bin/obj):
#   19 types implement IRunObserver - 5 in src/ (ConsoleRunObserver, LiveRunObserver,
#   OnTheFlyDiagramObserver, OnTheFlyLogSiteObserver, NullObserver), 5 in tests/Guardrails.Core.Tests
#   (RecordingObserver in EscalationSinkTests, OverwatchNoVerdictTests,
#   SchedulerBreakdownPhaseEventsTests and SchedulerDriftAutoResolveTests, plus CleanupSpyObserver in
#   TopologyM2CleanupTests - FIVE, not seven; there are no "two more"), and 9 in
#   tests/Guardrails.Integration.Tests.
# The ARGUMENT is unchanged and is what actually matters: the implementors are split across BOTH test
# projects, and building tests/Guardrails.Integration.Tests would not compile the five in
# tests/Guardrails.Core.Tests. A default-interface member is designed not to break them - that is the
# point of the default no-op body - but "designed not to" is a prediction, and this is the check that
# turns it into a measurement. Guardrails.sln is the smallest scope that compiles every implementor.
#
