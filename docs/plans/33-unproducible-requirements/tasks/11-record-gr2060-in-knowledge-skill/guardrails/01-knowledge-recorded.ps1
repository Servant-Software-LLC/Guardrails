...
#          GR2070 0, GR2071 0. All expected 0.
...
if ($doc -notmatch 'GR2060') {
    $failures.Add('GR2060 IS NOT RECORDED in ' + $subject + '. Name it and state the producer-coverage invariant: a guardrail may only require content some task in the plan can actually produce.')
}
if ($doc -notmatch 'GR2070') {
    ...
    if ($doc -notmatch '(?i)declin|held|never fired') {
        $failures.Add('GR2070 IS MENTIONED BUT NOT MARKED HELD in ' + $subject + '. A bare mention reads as an available code. Say it was declined, and give the bar for revisiting it: a defect, at a commit.')
    }
}
