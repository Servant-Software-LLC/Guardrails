...
:::warn
**The blend is what hides it.** The recorded hash is computed over both liveness classes at once. For the
...
:::note
**Review round 1 is settled.** All three questions below were answered on 2026-09-01 and each answer
...
:::note
**#556 has been reopened.** It was auto-closed at `1490d2a` by plan 31's own doc commit, whose body read
...
:::question
{ "id": "jit-wave-watch-blindness", "title": "The plan-edit watch is blind to JIT-authored waves — file it against #545, or fold it into this plan?",
  "mode": "single",
  "options": ["File it against #545's component", "Fold it into this plan", "Leave it recorded here only"],
  "recommended": "File it against #545's component",
  "rationale": "Scheduler.cs:143 builds LivePlanEditWatch from the run-start plan; SpliceAuthoredWave returns a NEW PlanDefinition and the field is never rebased, so a JIT-authored wave's tasks are invisible to the #545-part-3 warning, and LivePlanEditWatch.cs:95-100's adopt-silently branch looks unreachable in production. It is a gap in what shipped this morning, in #545's component, not in this contract - folding it in would mix a second defect into a drift-contract change, which is the exact reason plan 31 refused to absorb #556. Stated as a filing rather than a claim: it was found while evaluating reuse and has NOT been verified against the test suite.",
  "target": "human", "answer": ["File it against #545\u0027s component"] }
:::

:::question
{ "id": "gate-stops-dispatch", "title": "Should the divergence gate eventually stop dispatch, rather than only blocking delivery?",
  "mode": "single",
  "options": ["Defer to v2 - revisit only on measured evidence", "Build it in this plan", "Rule it out permanently"],
  "recommended": "Defer to v2 - revisit only on measured evidence",
  "rationale": "Declined for v1 in Risk 2 with reasons. Stopping dispatch mid-run means killing in-flight tasks and reasoning about partially-settled state, against a benefit nobody has measured: the cost of the work a run does AFTER a divergence it has already detected. That measurement does not exist yet. Ruling it out permanently would discard a real option on no more evidence than building it would.",
  "target": "human", "answer": ["Defer to v2 - revisit only on measured evidence"] }
:::

:::question
{ "id": "task-dir-escape", "title": "GUARDRAILS_TASK_DIR hands every action the MAIN checkout's task folder, outside the segment worktree - where does that get fixed?",
  "mode": "single",
  "options": ["File against #557's containment work", "Fold into this plan", "File as its own issue"],
  "recommended": "File against #557's containment work",
  "rationale": "TaskExecutor.cs:511 and :2029 hand every action and guardrail a path outside the segment worktree, so writes there are invisible to the write-scope check. TODAY that silently moves a recorded hash; AFTER this plan it halts a run - so this plan makes an existing hole louder rather than creating one, which is an argument for fixing it beside #557's containment work rather than inside a drift-contract change. Worth your judgment because the severity changes on the day this ships.",
  "target": "human", "answer": ["File against #557\u0027s containment work"] }
:::

:::note
**The #552 sequencing note is resolved.** It warned that this plan's stage 16 and the concurrent
