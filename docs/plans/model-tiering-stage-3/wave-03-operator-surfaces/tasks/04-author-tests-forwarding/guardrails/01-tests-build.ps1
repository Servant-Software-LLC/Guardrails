...
if ($buildExit -ne 0) {
    ...
    Write-Output "AttemptModelForwardingTests must COMPILE and FAIL. It does not compile - most likely it references a member no stub from task 01 declares. Task 01 owns those stubs; do NOT edit them from here."
    exit 1
}
exit 0