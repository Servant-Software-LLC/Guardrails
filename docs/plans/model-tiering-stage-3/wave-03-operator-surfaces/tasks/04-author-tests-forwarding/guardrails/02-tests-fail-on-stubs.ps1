...
#          here, green after 07-forward-attempt-model-in-decorators, and PASSES this census. That residual is a human read.
#
# SCOPE (#455): the filter names exactly the ONE class this task owns. Every behaviour it selects is made
# green by 07-forward-attempt-model-in-decorators - a task DOWNSTREAM of this one - so there is no
...
try {
    ...
    if ($results.Count -lt 1) {
        Write-Output "the filter FullyQualifiedName~AttemptModelForwardingTests selected ZERO tests - the class is missing, empty, or named differently. Nothing was measured, so nothing is proven red."
        exit 1
    }
    ...
    foreach ($name in $required) {
        ...
        if ($matched.Count -lt 1) {
            $failures += "'$name' was not executed at all - the prompt pins this method name and the census reads it. Either the test is missing, or it is not in AttemptModelForwardingTests"
        }
        elseif (@($matched | Where-Object { $_.outcome -eq 'Failed' }).Count -lt 1) {
            $failures += "'$name' ran but did NOT fail (outcome: $(($matched | ForEach-Object { $_.outcome }) -join ', ')) - a TDD red must FAIL while neither on-the-fly decorator declares AttemptModelResolved, so each silently inherits the empty default. Either the assertion is hollow, or it asserts something already true"
        }
    }
    ...
    if ($failures.Count -gt 0) {
        ...
        Write-Output "Every behaviour this task's prompt enumerates must be a NAMED test in AttemptModelForwardingTests that FAILS on this wave's entry tree."
        exit 1
    }
    exit 0
}
