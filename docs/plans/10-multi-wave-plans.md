# Architecture: first-class multi-wave plans (nested layout) — design of record (#254)

> **AS-BUILT (2026-07): v1 LANDED.** Both the foundation (M2a: nested-layout loader/validator,
> wave-qualified identity, `WaveDefinitionHash`, the journal `waves[]` schema) AND the wave-execution loop
> (M2b: one continuous integration worktree/journal/plan branch across waves behind hard barriers, per-wave
> entry/exit gates, the `Guardrails-Wave:` marker commit, cross-wave resume, runtime wave-drift resolution,
> wave-scoped reset) shipped to master. The "ship the skeleton as v1 / defer execution" framing in the
> draft status below is therefore **stale** — wave execution is live; `guardrails run` runs a waved plan
> wave by wave. **The as-built contract is SSOT `02-schemas-and-contracts.md` §14** (where the two differ,
> §14 wins). Auto-heal + overwatcher-driven inter-wave adjustment remain **v2 bets** (see
> `docs/plans/11-overwatcher.md` §9). The original PLANNED design-draft text is preserved verbatim below as
> the design record.

> **Status (original design draft): PLANNED — design of record for maintainer review (the #106 draft-PR loop).** The
