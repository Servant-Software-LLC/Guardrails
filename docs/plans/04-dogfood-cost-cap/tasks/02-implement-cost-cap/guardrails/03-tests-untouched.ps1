...
# not the agent sandbox, so it always works (no git, no allowedTools gate). The recorded hash
# is contract-protected: single-writer-per-key (SSOT §6.2) means no intervening task can forge
# 01-author-cost-cap-tests.fileHashes to poison this check.
...
$storedHashes = $state.'01-author-cost-cap-tests'.fileHashes

if (-not $storedHashes) {
  Write-Output "State key '01-author-cost-cap-tests.fileHashes' missing — was captureHashes declared on task 01?"
  exit 1
}
