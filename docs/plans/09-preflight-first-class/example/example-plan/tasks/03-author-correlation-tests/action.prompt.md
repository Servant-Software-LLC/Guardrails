...
yet. That red state is the point — it is what the `02-baseline-correlation-absent` preflight
already proved, and what this task's `tests-fail-on-current-code` guardrail re-asserts. The
later green (after `04-implement-correlation`) then PROVES the implementation did the work.
