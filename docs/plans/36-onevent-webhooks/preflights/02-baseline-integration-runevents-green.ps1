# catches: this plan building on a RED start. Tasks 08/09 modify RunCommand and its observer chain, the
#          surface Category=RunEvents in Guardrails.Integration.Tests covers. If those are ALREADY red on
...
# Measured baseline (#478): Category=RunEvents in Guardrails.Integration.Tests = 51 tests, all passing (measured on this branch).
...
if (($passed + $failed) -lt 1) {
    Write-Output "PRECONDITION: the Integration RunEvents filter executed ZERO tests - the filter, the trait or the test project moved. This preflight certifies nothing until it selects real tests."
    exit 1
}

if ($code -ne 0) {
    Write-Output ""
    Write-Output "=== Pre-existing Integration RunEvents failures (this plan has not run yet) ==="
    $log -split "`r?`n" | Where-Object {
        $_ -match '^\s*(Failed|Error Message|Expected|Actual|Stack Trace|\s+at )' -or $_ -match '\[FAIL\]'
    } | ForEach-Object { Write-Output $_ }
    Write-Output ""
    Write-Output "The Integration RunEvents tests are ALREADY failing on the starting code ($failed failed of $($passed + $failed) executed). Fix the pre-existing breakage before this plan builds on it - never build on red (#181)."
    exit 1
}

Write-Output "Baseline green: $passed Integration RunEvents test(s) pass on the starting code."
exit 0
