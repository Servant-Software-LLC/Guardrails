...
#          the harness recorded for 05-author-tests-git-worktree-lifecycle.captureHashes and recomputes
#          with Get-FileHash (interpreter cmdlet, no git, no allowedTools gate).
$state = Get-Content $env:GUARDRAILS_STATE_IN -Raw | ConvertFrom-Json
$storedHashes = $state.'05-author-tests-git-worktree-lifecycle'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '05-author-tests-git-worktree-lifecycle.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
