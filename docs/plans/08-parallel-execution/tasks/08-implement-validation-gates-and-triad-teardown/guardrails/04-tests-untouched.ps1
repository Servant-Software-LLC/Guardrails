...
#          the harness recorded for 07-author-tests-validation-gates-and-triad-removal.captureHashes
#          and recomputes with Get-FileHash (interpreter cmdlet, no git).
$state = Get-Content $env:GUARDRAILS_STATE_IN -Raw | ConvertFrom-Json
$storedHashes = $state.'07-author-tests-validation-gates-and-triad-removal'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '07-author-tests-validation-gates-and-triad-removal.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
