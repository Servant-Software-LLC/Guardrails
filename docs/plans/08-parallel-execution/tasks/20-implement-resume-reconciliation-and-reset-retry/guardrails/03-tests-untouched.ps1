...
#          the harness recorded for 19-author-tests-resume-and-reset-retry.captureHashes and recomputes
#          with Get-FileHash (interpreter cmdlet, no git).
$state = Get-Content $env:GUARDRAILS_STATE_IN -Raw | ConvertFrom-Json
$storedHashes = $state.'19-author-tests-resume-and-reset-retry'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '19-author-tests-resume-and-reset-retry.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
