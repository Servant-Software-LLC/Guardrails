...
#          the harness recorded for 23-author-tests-guardrail-scope-field.captureHashes and recomputes
#          with Get-FileHash (interpreter cmdlet, no git).
$state = Get-Content $env:GUARDRAILS_STATE_IN -Raw | ConvertFrom-Json
$storedHashes = $state.'23-author-tests-guardrail-scope-field'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '23-author-tests-guardrail-scope-field.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
