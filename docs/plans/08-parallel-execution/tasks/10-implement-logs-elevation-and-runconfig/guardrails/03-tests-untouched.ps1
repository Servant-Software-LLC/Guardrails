...
#          the harness recorded for 09-author-tests-logs-elevation-and-runconfig.captureHashes and
#          recomputes with Get-FileHash (interpreter cmdlet, no git).
$state = Get-Content $env:GUARDRAILS_STATE_IN -Raw | ConvertFrom-Json
$storedHashes = $state.'09-author-tests-logs-elevation-and-runconfig'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '09-author-tests-logs-elevation-and-runconfig.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
