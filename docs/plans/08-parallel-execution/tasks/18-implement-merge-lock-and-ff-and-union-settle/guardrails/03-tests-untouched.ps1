...
#          the harness recorded for 17-author-tests-merge-lock-and-ff-and-union-settle.captureHashes
#          and recomputes with Get-FileHash (interpreter cmdlet, no git).
$state = Get-Content $env:GUARDRAILS_STATE_IN -Raw | ConvertFrom-Json
$storedHashes = $state.'17-author-tests-merge-lock-and-ff-and-union-settle'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '17-author-tests-merge-lock-and-ff-and-union-settle.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
