...
#          the harness recorded for 25-author-tests-ai-merge-worker.captureHashes and recomputes with
#          Get-FileHash (interpreter cmdlet, no git).
$state = Get-Content $env:GUARDRAILS_STATE_IN -Raw | ConvertFrom-Json
$storedHashes = $state.'25-author-tests-ai-merge-worker'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '25-author-tests-ai-merge-worker.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
