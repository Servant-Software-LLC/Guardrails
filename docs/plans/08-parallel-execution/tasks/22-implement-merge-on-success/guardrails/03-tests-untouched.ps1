...
#          the harness recorded for 21-author-tests-merge-on-success.captureHashes and recomputes with
#          Get-FileHash (interpreter cmdlet, no git).
$state = Get-Content $env:GUARDRAILS_STATE_IN -Raw | ConvertFrom-Json
$storedHashes = $state.'21-author-tests-merge-on-success'.fileHashes
if (-not $storedHashes) {
    Write-Output "State key '21-author-tests-merge-on-success.fileHashes' missing - was captureHashes declared on the test-author task?"
    exit 1
}
$failures = @()
