...
if ($ran -lt 1) {
    Write-Output "ZERO tests executed - the TDD-red proof certified nothing. MOST LIKELY CAUSE: the tests are missing the class-level Trait attribute for Category = TierResolution, which is what this filter selects on - check that FIRST, and adding it IS an in-scope fix to your own test file. Other causes: the class is not named JudgeProvenanceSchemaTests, the filter is malformed, every matched test is [Skip]ped, or the test host failed to start."
    exit 1
}

if ($testExit -eq 0) {
    Write-Output ""
    Write-Output "every JudgeProvenanceSchemaTests test PASSED against the stub - so the suite asserts nothing. MOST LIKELY CAUSE: the suite only asserts the record's shape, which is true the moment the stub is declared. Assert the SERIALIZATION behaviour instead - that the key is ABSENT from the emitted JSON text when null, that it round-trips when present, and that an older journal without the key still reads."
    exit 1
}
exit 0
