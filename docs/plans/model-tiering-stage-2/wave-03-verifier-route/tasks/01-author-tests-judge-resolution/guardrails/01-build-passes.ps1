...
if ($LASTEXITCODE -ne 0) {
    Write-Output "the solution does not compile - JudgeResolutionTests must COMPILE and FAIL, not fail to compile. Read the compile errors above: they name the exact symbol. If the missing symbol is OUTSIDE your write scope, do not edit that file - emit a needsHuman fragment to the state-out path instead."
    exit 1
}
exit 0
