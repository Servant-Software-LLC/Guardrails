...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `wave-03-verifier-route/10-implement-verifier-advisory`, NOT the stableId and NOT
  the bare folder name. The harness REJECTS a fragment keyed by anything else (every
  attempt), so:
  `{ "wave-03-verifier-route/10-implement-verifier-advisory": { "someKey": "someValue" } }`.
...
Implement **`VerifierAdvisory`** so the tests authored by `09-author-tests-verifier-advisory` pass.
