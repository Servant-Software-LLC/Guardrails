# catches: a selection implementation whose behavior deviates from the tests THIS task pair owns. The
#          --filter names this pair's OWN test class, never the plan-wide trait alone - a trait-only
#          filter asserts the state of every test in the plan, so this task could not go green until a
#          task that DEPENDS on it had run (a deadlock validate/graph --check cannot see, #455).
...
if ($testExit -ne 0) {
    ...
    Write-Output "TierResolverCandidateSelectionTests failing - candidate selection is not implemented to DoR 6.2 (see failure details above)"
    exit 1
}
