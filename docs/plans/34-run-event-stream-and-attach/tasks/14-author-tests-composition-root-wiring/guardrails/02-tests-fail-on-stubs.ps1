...
# DECLARED EXEMPTION: 'BuildObserverChain_StillWiresTheExistingObservers' - task 13 extracted the seam WITHOUT changing what it builds, so the pre-existing observers are already wired and a CORRECT contrast test is GREEN on the stub tree. The row asserts Expect='Executed'
#          (it ran, and was not [Skip]ped) rather than Failed; it stays IN the manifest because a
...
$filter = 'Category=RunEvents&FullyQualifiedName~RunCommandObserverWiringTests'   # SAME string as the pair's forward half
...
$manifest = [ordered]@{
    'the chain constructs the events projection'                                     = 'BuildObserverChain_ConstructsTheEventsProjection'
    'the chain constructs the observer projection'                                   = 'BuildObserverChain_ConstructsTheObserverProjection'
    'both projections are wired in the --no-ui branch'                               = 'BuildObserverChain_WiresBothProjections_InTheNoUiBranch'
    'both projections are wired in the live-UI branch'                               = 'BuildObserverChain_WiresBothProjections_InTheLiveUiBranch'
    'an attempt raised through the REAL composed chain reaches events.jsonl on disk' = 'AttemptFinished_ThroughTheRealComposedChain_WritesEventsJsonl'
    'the chain still wires the observers it already had (contrast case)'             = @{ Name = 'BuildObserverChain_StillWiresTheExistingObservers'; Expect = 'Executed' }
}
