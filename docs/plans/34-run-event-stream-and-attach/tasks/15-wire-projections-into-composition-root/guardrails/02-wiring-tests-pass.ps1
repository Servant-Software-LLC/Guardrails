...
$filter = 'Category=RunEvents&FullyQualifiedName~RunCommandObserverWiringTests'
...
if ($testExit -ne 0) {
    ...
    Write-Output "RunCommandObserverWiringTests failing - the production observer chain does not construct and inject both projections (see failure details above)"
    exit 1
}
