...
$filter = 'Category=RunEvents&FullyQualifiedName~AttachReplayTests'   # SAME string as the pair's forward half
...
$manifest = [ordered]@{
    'attach drives the REAL LiveRunObserver from observer.jsonl'        = 'Attach_DrivesTheRealLiveRunObserver_FromObserverJsonl'
    'attach replays the recorded call sequence in order'                = 'Attach_ReplaysTheRecordedCallSequence_InOrder'
    'attaching to a finished run replays to completion'                 = 'Attach_OnAFinishedRun_ReplaysToCompletion'
    'two concurrent attachments both replay, neither writes to the run' = 'TwoConcurrentAttachments_BothReplayEveryEvent_AndNeitherWritesToTheRun'
    'a missing observer.jsonl fails with an actionable message'         = 'Attach_OnAMissingObserverJsonl_FailsWithAnActionableMessage'
}
