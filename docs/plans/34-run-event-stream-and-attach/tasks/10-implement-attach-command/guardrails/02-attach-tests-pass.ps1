...
$filter = 'Category=RunEvents&FullyQualifiedName~AttachReplayTests'
...
if ($testExit -ne 0) {
    ...
    Write-Output "AttachReplayTests failing - guardrails attach does not replay observer.jsonl into the real LiveRunObserver (see failure details above)"
    exit 1
}
