...
$filter = 'Category=RunEvents&FullyQualifiedName~EventsEndpointTests'
...
if ($testExit -ne 0) {
    ...
    Write-Output "EventsEndpointTests failing - the /events route does not stream events.jsonl as specified, or an existing route regressed (see failure details above)"
    exit 1
}
