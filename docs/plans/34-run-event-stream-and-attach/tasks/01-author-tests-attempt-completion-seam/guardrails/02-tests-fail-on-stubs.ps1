...
$filter = 'Category=RunEvents&FullyQualifiedName~AttemptCompletionForwardingTests'   # SAME string as the pair's forward half
...
$manifest = [ordered]@{
    'the log-site decorator forwards AttemptFinished'          = 'OnTheFlyLogSiteObserver_ForwardsAttemptFinished'
    'the diagram decorator forwards AttemptFinished'           = 'OnTheFlyDiagramObserver_ForwardsAttemptFinished'
    'the forwarded AttemptOutcome is the one it was given'     = 'OnTheFlyLogSiteObserver_ForwardsOutcomeVerbatim'
    'every decorator forwards for EVERY AttemptOutcome member' = 'EveryDecorator_ForwardsAttemptFinished_ForEveryOutcome'
}
