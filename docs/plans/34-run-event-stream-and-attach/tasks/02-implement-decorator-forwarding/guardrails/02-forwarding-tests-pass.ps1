...
$filter = 'Category=RunEvents&FullyQualifiedName~AttemptCompletionForwardingTests'
...
if ($testExit -ne 0) {
    ...
    Write-Output "AttemptCompletionForwardingTests failing - one or more IRunObserver implementations do not forward AttemptFinished (see failure details above)"
    exit 1
}
