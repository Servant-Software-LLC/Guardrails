...
# DECLARED EXEMPTION: 'LogServer_StillServesItsExistingRoutes' - the EXISTING /tasks and /diagram.html routes already work before task 12 touches LogServer, so a CORRECT regression test is GREEN on the stub tree. The row asserts Expect='Executed'
...
$filter = 'Category=RunEvents&FullyQualifiedName~EventsEndpointTests'   # SAME string as the pair's forward half
...
$manifest = [ordered]@{
    '/events streams existing events to a late subscriber'    = 'EventsEndpoint_StreamsExistingEventsToALateSubscriber'
    '/events streams subsequent events as they are appended'  = 'EventsEndpoint_StreamsSubsequentEventsAsTheyAreAppended'
    '/events emits one parseable event per message'           = 'EventsEndpoint_EmitsOneParseableEventPerMessage'
    'a missing events.jsonl is an empty stream, not an error' = 'EventsEndpoint_OnAMissingEventsFile_ReturnsAnEmptyStreamNotAnError'
    'the existing LogServer routes still work'                = @{ Name = 'LogServer_StillServesItsExistingRoutes'; Expect = 'Executed' }
}
