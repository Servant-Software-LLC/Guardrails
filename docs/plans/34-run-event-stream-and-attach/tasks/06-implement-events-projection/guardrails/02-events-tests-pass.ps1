...
$filter = 'Category=RunEvents&FullyQualifiedName~RunEventStreamTests'
...
if ($testExit -ne 0) {
    ...
    Write-Output "RunEventStreamTests failing - the events.jsonl projection does not emit the agreed row shape or does not forward (see failure details above)"
    exit 1
}
