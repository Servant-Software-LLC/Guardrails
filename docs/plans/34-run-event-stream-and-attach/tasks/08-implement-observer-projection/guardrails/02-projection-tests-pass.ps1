...
$filter = 'Category=RunEvents&FullyQualifiedName~ObserverProjectionTests'
...
if ($testExit -ne 0) {
    ...
    Write-Output "ObserverProjectionTests failing - observer.jsonl does not record or replay the observed call sequence faithfully (see failure details above)"
    exit 1
}
