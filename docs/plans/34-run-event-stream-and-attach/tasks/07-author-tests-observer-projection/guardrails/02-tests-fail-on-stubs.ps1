...
$filter = 'Category=RunEvents&FullyQualifiedName~ObserverProjectionTests'   # SAME string as the pair's forward half
...
$manifest = [ordered]@{
    'every observed call appends one line naming the member'  = 'EveryObservedCall_AppendsOneLine_NamingTheMember'
    'replay reproduces the observed call sequence in order'   = 'Replay_ReproducesTheObservedCallSequence_InOrder'
    'AttemptFinished is projected with its outcome'           = 'AttemptFinished_IsProjected_WithItsOutcome'
    'the decorator forwards every observed call to the inner' = 'Decorator_ForwardsEveryObservedCallToTheInner'
    'two concurrent readers both read every line'             = 'TwoConcurrentReaders_BothReadEveryLine'
}
