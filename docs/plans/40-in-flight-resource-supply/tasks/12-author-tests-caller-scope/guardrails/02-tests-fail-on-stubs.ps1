...
$pinned = @(
    'OperatorInvocation_IsUnrestricted',
    'TaskInvocation_MaySupplyInsideItsOwnWriteScope',
    'TaskInvocation_IsRefusedOutsideItsWriteScope',
    'TaskInvocation_UsesTheSameMembershipRuleAsTheWriteScopeCheck'
)
...
try {
    ...
    & dotnet test "tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj" -c Debug --nologo `
        --filter "FullyQualifiedName~SupplyCallerScopeTests" `
    ...
    if ($results_nodes.Count -lt 1) {
        ...
        Write-Output "PRECONDITION: the filter 'FullyQualifiedName~SupplyCallerScopeTests' matched NO tests. A zero-match filter exits 0 and certifies nothing — fix the filter or the class name."
        exit 1
    }
