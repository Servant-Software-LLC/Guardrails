...
try {
    ...
    & dotnet test "tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj" -c Debug --nologo `
        --filter "FullyQualifiedName~OverwatchSupplyAutoResolveTests" `
    ...
    if ($results_nodes.Count -lt 1) {
        ...
        Write-Output "PRECONDITION: the filter 'FullyQualifiedName~OverwatchSupplyAutoResolveTests' matched NO tests. A zero-match filter exits 0 and certifies nothing — fix the filter or the class name."
        exit 1
    }
