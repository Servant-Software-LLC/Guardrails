...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `11-implement-supply-command`), NOT the stableId. The harness REJECTS a fragment
  keyed by anything else (every attempt), so:
  `{ "11-implement-supply-command": { "someKey": "someValue" } }`.
...
Registration is not a detail — the tests drive the real root, so an unregistered command fails
at parse. Design 40 §6 is the contract; the refusal rule is narrow and deliberate: refuse only
when there is **no resumable run at all**, never merely because no run is executing.
