...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `13-implement-caller-scope`), NOT the stableId. The harness REJECTS a fragment
  keyed by anything else (every attempt), so:
  `{ "13-implement-caller-scope": { "someKey": "someValue" } }`.
...
Fill real logic over the stub in `src/Guardrails.Core/Execution/SupplyCallerScope.cs` so
`SupplyCallerScopeTests` passes.

Reuse `WriteScope.IsInScope` for the membership test — do not re-implement it. The design is
explicit that the scoping rule must agree with the check the harness already enforces at write
time.
