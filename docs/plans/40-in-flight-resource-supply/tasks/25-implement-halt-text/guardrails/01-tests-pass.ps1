# catches: an implementation that does not actually satisfy the tests authored upstream — and,
#          via the zero-match guard, a filter that silently selects nothing (#455).
$ErrorActionPreference = 'Stop'
$PSNativeCommandUseErrorActionPreference = $false

$env:DOTNET_CLI_UI_LANGUAGE = 'en'

$out = & dotnet test "tests/Guardrails.Integration.Tests/Guardrails.Integration.Tests.csproj" -c Debug --nologo --filter "FullyQualifiedName~SuppliedHaltTextTests" 2>&1 | Out-String
$code = $LASTEXITCODE
...
if ($code -ne 0) {
    ...
    # output: an allowlist drops the `Failed <TestName>` header and `System.NotImplementedException`.
    $inBlock = $false
    $emitted = 0
...
if (($passed + $failed) -lt 1) {
    Write-Output "ZERO-MATCH: the filter 'FullyQualifiedName~SuppliedHaltTextTests' executed no tests. That exits 0 from dotnet test and would certify this task on an empty set."
    exit 1
}
