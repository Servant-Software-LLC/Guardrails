...
if (($passed + $failed) -lt 1) {
    Write-Output "ZERO-MATCH: the filter 'FullyQualifiedName~SuppliedObserverCliForwardingTests' executed no tests. That exits 0 from dotnet test and would certify this task on an empty set."
    exit 1
}
