...
#          scoped to the TaskBoundary_ rows only (#455). This task and task 16 both used the
#          whole-class filter while each owned only one of the two wiring sites, and each
#          sibling's guardrails run in its OWN segment, which never contains the other's file.
#          That was a mutual deadlock, not merge-order luck: neither could ever go green, and
#          neither could fix it (the other's file is out of scope). `guardrails validate` and
#          `graph --check` both PASS on it, because the cycle is task-to-sibling-test-corpus
#          rather than task-to-task. Task 16 now dependsOn this task and keeps the class-wide
#          filter, where it is honest.
...
$out = & dotnet test "tests/Guardrails.Integration.Tests/Guardrails.Integration.Tests.csproj" -c Debug --nologo --filter "FullyQualifiedName~SuppliedBoundaryWiringTests.TaskBoundary_" 2>&1 | Out-String
$code = $LASTEXITCODE
...
if (($passed + $failed) -lt 1) {
    Write-Output "ZERO-MATCH: the filter 'FullyQualifiedName~SuppliedBoundaryWiringTests.TaskBoundary_' executed no tests. That exits 0 from dotnet test and would certify this task on an empty set."
    exit 1
}
