...
$pinned = @(
    'Supply_StagesTheFileUnderLogsRunIdSupplied',
...
try {
    ...
    & dotnet test "tests/Guardrails.Integration.Tests/Guardrails.Integration.Tests.csproj" -c Debug --nologo `
        --filter "FullyQualifiedName~SupplyCommandTests" `
    ...
    if ($results_nodes.Count -lt 1) {
        ...
        Write-Output "PRECONDITION: the filter 'FullyQualifiedName~SupplyCommandTests' matched NO tests. A zero-match filter exits 0 and certifies nothing — fix the filter or the class name."
        exit 1
    }
