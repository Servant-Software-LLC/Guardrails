...
$out = & dotnet test "tests/Guardrails.Integration.Tests/Guardrails.Integration.Tests.csproj" -c Debug --nologo --filter "FullyQualifiedName~SupplyResumeShorthandTests" 2>&1 | Out-String
$code = $LASTEXITCODE
...
if (($passed + $failed) -lt 1) {
    Write-Output "ZERO-MATCH: the filter 'FullyQualifiedName~SupplyResumeShorthandTests' executed no tests. That exits 0 from dotnet test and would certify this task on an empty set."
    exit 1
}
