...
- Write everything you publish under your task's FOLDER NAME as the single top-level
  ...
  `18-implement-resume-shorthand`), NOT the stableId. The harness REJECTS a fragment
  keyed by anything else (every attempt), so:
  `{ "18-implement-resume-shorthand": { "someKey": "someValue" } }`.
...
Implement `--resume` on the supply verb so `SupplyResumeShorthandTests` passes.

**Share the code path; do not parallel-implement it.** The tests compare the journal and the
