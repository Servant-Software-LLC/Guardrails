...
#          core suite covers, so a pre-existing failure would be misattributed to the
...
$out = & dotnet test "tests/Guardrails.Core.Tests/Guardrails.Core.Tests.csproj" -c Debug --nologo --filter "Category!=Supply" 2>&1 | Out-String
$code = $LASTEXITCODE
