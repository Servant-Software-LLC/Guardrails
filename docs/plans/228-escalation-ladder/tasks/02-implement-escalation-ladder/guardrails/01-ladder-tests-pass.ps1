# catches: an implementation whose behavior deviates from the tests THIS task pair owns - chiefly the
#          three cap-and-degrade cases (top rung, single-runner legacy config, no-route above), which a
#          ladder that always climbs would fail and a ladder that never climbs would pass. The --filter
...
$filter = 'Category=EscalationLadder&FullyQualifiedName~EscalationLadderTests'   # verbatim from task 01's census
...
if ($testExit -ne 0) {
    ...
    Write-Output "EscalationLadderTests failing - the rung-selection ladder is not implemented to spec (see failure details above)"
    exit 1
}
...
if ($ran -lt 1) {
    Write-Output "exit 0 but ZERO tests executed - this guardrail certified nothing. The --filter '$filter' matched no tests, is malformed, or every matched test is [Skip]ped. Check it against EscalationLadderTests, the class task 01 authored."
    exit 1
}
exit 0
