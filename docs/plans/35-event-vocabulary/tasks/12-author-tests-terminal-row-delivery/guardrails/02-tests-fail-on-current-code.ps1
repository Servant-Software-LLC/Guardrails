...
#          An invoking-then-hollow test is red here, green after, and passes. The exempt test asserts the deliberate empty-200 behaviour that must NOT change; demanding it be red would demand a correct implementation fail.
$ErrorActionPreference = 'Continue'
$env:DOTNET_CLI_UI_LANGUAGE = 'en'

$filter = "FullyQualifiedName~EventsStreamShutdownTests"
...
try {
    ...
    $mustFail = @(
        'ASubscriberReceivesARowAppendedJustBeforeShutdown'
    )
    ...
    $mustExecute = @(
        'AMissingEventsFileStillCompletesWithAnEmptyBody'
    )
