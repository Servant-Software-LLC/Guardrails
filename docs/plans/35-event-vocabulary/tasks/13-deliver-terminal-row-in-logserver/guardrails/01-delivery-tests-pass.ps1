# catches: a shutdown path that still returns without a final read, so the terminal row -
#          run-finished, the entire payoff of this plan - lands in the file and never reaches the
#          wire.
...
Write-Output "All $passed test(s) pass for EventsStreamShutdownTests."
exit 0
