...
#          An invoking-then-hollow test is red here, green after, and passes. Here that boundary is unusually sharp: a test asserting only attach's EXIT CODE would be red now and green after while proving nothing, because AttachCommand exits 0 when it skipped every line.
...
try {
    ...
    $mustFail = @(
        'ObserverLine_CarriesEveryRequiredAttemptRecordMember',
        'AttachReplaysTheAttempt_RatherThanSilentlySkippingIt',
        'AttachReplay_ReconstructsTheRecordFields',
        'RunFinishedIsRecordedOnTheObserverStream'
    )
