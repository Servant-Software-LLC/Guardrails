# catches: a writer that satisfies some of the vocabulary and silently drops the rest - the
#          field set, the run-scoped row with no taskId, the seq ordering key, and the
#          constructor-supplied runId are one deliverable, and a partial implementation leaves a
#          consumer unable to trust any of them.
...
Write-Output "All $passed test(s) pass for RunEventVocabularyTests."
exit 0
