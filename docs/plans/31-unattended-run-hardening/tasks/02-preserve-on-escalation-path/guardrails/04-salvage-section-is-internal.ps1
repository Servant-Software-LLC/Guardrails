...
#   $env:GR_SUBJECT='docs/plans/31-unattended-run-hardening/tasks/02-preserve-on-escalation-path/samples/04-salvage-section-is-internal.valid.cs';   <this script>  # expect 0
#   $env:GR_SUBJECT='docs/plans/31-unattended-run-hardening/tasks/02-preserve-on-escalation-path/samples/04-salvage-section-is-internal.invalid.cs'; <this script>  # expect 1
# RE-RUN EVERY case after ANY edit to this file, not just the clause you touched.
$rel  = if ($env:GR_SUBJECT) { $env:GR_SUBJECT } else { 'src/Guardrails.Core/Execution/RetryPolicy.cs' }
