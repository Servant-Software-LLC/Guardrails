...
#   $env:GR_SUBJECT='docs/plans/31-unattended-run-hardening/tasks/05-implement-handoff-coverage-check/samples/03-no-second-glob-matcher.valid.cs';   <this script>  # expect 0
#   $env:GR_SUBJECT='docs/plans/31-unattended-run-hardening/tasks/05-implement-handoff-coverage-check/samples/03-no-second-glob-matcher.invalid.cs'; <this script>  # expect 1
# RE-RUN EVERY case after ANY edit to this file, not just the clause you touched.
$rel  = if ($env:GR_SUBJECT) { $env:GR_SUBJECT } else { 'src/Guardrails.Core/Loading/HandoffScopeCoverage.cs' }
