...
:::question
{"id": "gr2070-declined", "title": "GR2070 is declined — it has never fired on a real defect at any commit in this repo. Confirm, or overturn?", "mode": "single", "options": ["Confirm the decline — hold GR2070 by name", "Overturn — build it anyway on the cost-of-failure argument", "Retire the code entirely rather than holding it"], "recommended": "Confirm the decline — hold GR2070 by name", "rationale": "The lead session first decided to build it, then reversed on a git trace an adversarial pass produced and the lead session verified at the source. At 10816fb the motivating clause is 'pending\\s*\\.\\s*Bucket' — single-quoted, no named argument. The scope was fixed at 62d7314; the named-argument form first appears at 124a7d0, 36 minutes AFTER that fix, and single-quoted, so the shipped extractor reads it unchanged; it became double-quoted 48 minutes later in an unrelated refactor. The two halves of the predicate never coexist, so its true-positive population over the whole history is zero, and the double-quote relaxation it needed rested on a cosmetic edit. Holding the code by name keeps the three durable findings (universal quantification, the qualifier IS the check, a parameter list is readable where a member set is not) without spending a code on an unfalsified idea.", "target": "human"}
:::

:::question
{"id": "jit-veto-mitigation", "title": "How is the #501 veto mitigated — an allow-list entry keyed on wavePrefixIsIncomplete, or a stronger PlanIsClosed?", "mode": "single", "options": ["Allow-list entry keyed on wavePrefixIsIncomplete", "Strengthen PlanIsClosed to detect an un-authored wave", "Ship GR2060 at WARNING and defer the mitigation"], "recommended": "Allow-list entry keyed on wavePrefixIsIncomplete", "rationale": "Verified in the tree: PlanValidator.cs:3395 is Waves.All(w => w.Tasks.Count > 0), which sees an empty stub wave but returns true for an authored partial prefix; Scheduler.cs:2325's UnsatisfiableWhileIncomplete is a single-code comparison, and blocking = errors.Except(excused). So GR2060 at ERROR would fire on an incomplete scope union, go unexcused, veto, and revert the prefix wholesale — the exact defect #501 fixed. wavePrefixIsIncomplete is actual knowledge that folders are still owed and already reaches ValidatePlanAfterBreakdown; PlanIsClosed only infers completeness from shape. Filed as #583 for the general trap, which outlives this plan. Option 3 is the fallback if task 6 cannot land, and §5.5 makes ERROR conditional on it either way.", "target": "human"}
:::

:::question
{"id": "carrier-shape-cover", "title": "With GR2070 declined, the plan-30 carrier shape is covered only procedurally. Is one authoring step in two skills enough?", "mode": "single", "options": ["Yes — one step plus the falsification trigger, as designed", "No — Milestone C must ship something with teeth", "Accept the gap and record it on #474 only"], "recommended": "Yes — one step plus the falsification trigger, as designed", "rationale": "The shipped review probe walks UPSTREAM to the carrier, so for plan 30's clause it reaches RunReport.cs, finds it in scope, returns 'reachable; stop' — and never reads the callee's parameter list, where the defect was. Whatever caught it was improvisation, not procedure. Milestone C adds exactly that missing step. It is honestly weaker than a lint, and the plan says so: the alternative on offer was a lint with no positive control, which is worse than an honest gap. The falsification trigger is explicit — a second carrier-shaped defect reaching a run after C ships means the step is not being executed, and the answer is then a stronger authoring artifact, not a smarter lint.", "target": "human"}
:::
