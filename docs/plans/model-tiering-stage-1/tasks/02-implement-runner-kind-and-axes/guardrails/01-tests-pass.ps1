...
if ($ran -lt 1) {
    Write-Output "exit 0 but ZERO tests executed - this guardrail certified NOTHING. The --filter '$filter' matched no tests, is malformed, or every matched test is [Skip]ped. Was PromptRunnerSchemaTests renamed, or its Category trait dropped?"
    exit 1
}
exit 0
