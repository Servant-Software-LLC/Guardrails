...
if ($total -lt 1) {
    Write-Output "FILTER MATCHED NO TESTS ($filter) - this guardrail verified NOTHING (dotnet test ran 0 tests and exited 0). Was RegistryKindDispatchTests renamed, or its Category trait dropped?"
    exit 1
}
