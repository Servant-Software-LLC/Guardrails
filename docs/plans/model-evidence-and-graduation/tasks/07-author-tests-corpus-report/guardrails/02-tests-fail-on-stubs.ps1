...
$filter = 'Category=ModelEvidence&FullyQualifiedName~TelemetryReportTests'
$behaviours = @(
    @{ Name = 'stratified by model, tier and fingerprint bucket'; Test = 'Report_StratifiesByModelAndTierAndBucket' },
    @{ Name = 'below minimum n renders insufficient evidence';   Test = 'Report_BelowMinimumSample_RendersInsufficientEvidence' },
    @{ Name = 'attempts-to-green never without abandonment rate'; Test = 'Report_AttemptsToGreen_AlwaysAccompaniedByAbandonmentRate' },
    @{ Name = 'every row carries its sample size';               Test = 'Report_EveryRowCarriesItsSampleSize' },
    @{ Name = 'a costless provider reports no money, not zero';  Test = 'Report_CostlessProvider_ReportsNoMoney_NotZero' },
    @{ Name = 'different model fingerprints are never pooled';   Test = 'Report_DifferentModelFingerprints_AreNeverPooled' }
)
