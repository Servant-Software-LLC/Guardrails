...
$filter = 'Category=ModelEvidence&FullyQualifiedName~TelemetryCommandTests'   # VERBATIM from the pair's inverse half
...
if ($testExit -ne 0) {
    ...
    Write-Output "TelemetryCommandTests failing - the telemetry verb is not implemented to the spec those tests pin (see failure details above)"
    exit 1
}
...
if ($ran -lt 1) {
    Write-Output "exit 0 but ZERO tests executed - this guardrail certified nothing. The --filter '$filter' matched no tests, is malformed, or every matched test is [Skip]ped. Check it against the tests this pair owns (class TelemetryCommandTests, trait Category=ModelEvidence)."
    exit 1
}
exit 0
