...
$filter = 'Category=ModelEvidence&FullyQualifiedName~TelemetryIngestTests'
$behaviours = @(
    @{ Name = 'one task row per task per run';                   Test = 'Ingest_EmitsOneTaskRowPerTaskPerRun' },
    @{ Name = 'one attempt row per attempt, retries included';   Test = 'Ingest_EmitsOneAttemptRowPerAttempt_RetriesIncluded' },
    @{ Name = 'route provenance carried onto the attempt row';   Test = 'Ingest_CarriesRouteProvenanceOntoTheAttemptRow' },
    @{ Name = 'unreported cost and tokens stay null, not zero';  Test = 'Ingest_UnreportedCostAndTokens_StayNull_NotZero' },
    @{ Name = 're-ingesting the same run adds no rows';          Test = 'Ingest_SameRunTwice_AddsNoDuplicateRows' },
    @{ Name = 'guardrail-failed rows carry the classified kind'; Test = 'Ingest_GuardrailFailedRows_CarryTheClassifiedFailureKind' }
)
