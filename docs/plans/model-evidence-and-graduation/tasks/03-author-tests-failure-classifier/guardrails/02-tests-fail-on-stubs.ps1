...
$filter = 'Category=ModelEvidence&FullyQualifiedName~TelemetryFailureClassifierTests'
$behaviours = @(
    @{ Name = 'write-scope violation, from the feedback text';   Test = 'Classify_WriteScopeViolation_FromFeedbackText' },
    @{ Name = 'staging-move failure is NOT write-scope';         Test = 'Classify_StagingMoveFailure_IsNotWriteScope' },
    @{ Name = 'harness-write out-of-scope is its own kind';      Test = 'Classify_HarnessWriteOutOfScope_IsItsOwnKind' },
    @{ Name = 'a real guardrail failure stays guardrail-failed'; Test = 'Classify_RealGuardrailFailure_StaysGuardrailFailed' },
    @{ Name = 'missing log site is undifferentiated, not guessed'; Test = 'Classify_MissingLogSite_IsUndifferentiated_NeverGuessed' },
    @{ Name = 'unrecognized wording is undifferentiated';        Test = 'Classify_UnrecognizedFeedbackWording_IsUndifferentiated' }
)
