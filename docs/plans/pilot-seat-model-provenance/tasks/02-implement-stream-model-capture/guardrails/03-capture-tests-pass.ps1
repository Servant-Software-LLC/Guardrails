...
Set-Location $ws
$out = dotnet test tests/Guardrails.Core.Tests --filter "FullyQualifiedName~ClaudeStreamParserModelTests" --nologo 2>&1
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Output 'ClaudeStreamParserModelTests still failing (see above).'
    exit 1
}
exit 0
