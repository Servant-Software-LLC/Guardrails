...
Set-Location $ws
$out = dotnet test tests/Guardrails.Integration.Tests --filter "FullyQualifiedName~ObserverModelResolvedTests" --nologo 2>&1
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Output 'ObserverModelResolvedTests still failing (see above).'
    exit 1
}
exit 0
