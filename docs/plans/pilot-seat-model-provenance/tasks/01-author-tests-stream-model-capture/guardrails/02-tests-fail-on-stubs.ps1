...
Set-Location $ws
dotnet test tests/Guardrails.Core.Tests --filter "FullyQualifiedName~ClaudeStreamParserModelTests" --nologo
if ($LASTEXITCODE -eq 0) {
    Write-Output 'the ClaudeStreamParserModelTests PASS against the stubs - tautological (no real behavior asserted).'
    exit 1
}
exit 0
