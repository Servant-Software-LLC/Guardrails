...
/// A target is the wall only on an attempt that refused it (#534 / #708).
/// </summary>
public sealed class PermissionWallTrackerTests
{
    ...
    public void NonStructuralPath_RefusedOnce_DoesNotHalt()
    {
        ...
        var tracker = new PermissionWallTracker();
        tracker.Observe(1, ["src/protected/Secret.cs"]);
        Assert.False(tracker.ShouldHalt().Halt);
    }
    ...
    public void NonStructuralPath_RefusedOnTwoAttempts_HaltsAsRepeated()
    {
        ...
        var tracker = new PermissionWallTracker();
        tracker.Observe(1, ["src/protected/Secret.cs"]);   // attempt 1
        Assert.False(tracker.ShouldHalt().Halt);

        tracker.Observe(2, ["src/protected/Secret.cs"]);   // attempt 2 — same wall
        PermissionWallDecision decision = tracker.ShouldHalt();
        Assert.True(decision.Halt);
    ...
    public void DifferentPathsEachAttempt_DoNotCountAsARepeat()
    {
        ...
        var tracker = new PermissionWallTracker();
        tracker.Observe(1, ["src/a/One.cs"]);
        tracker.Observe(2, ["src/b/Two.cs"]);
        Assert.False(tracker.ShouldHalt().Halt);
    }
    ...
    public void SamePathRefusedManyTimesInOneAttempt_CountsAsOneAttempt_NotARepeat()
    {
        ...
        var tracker = new PermissionWallTracker();
        tracker.Observe(1, ["src/a/One.cs"]);   // a single attempt
        Assert.False(tracker.ShouldHalt().Halt);
    }
    ...
    {
        var tracker = new PermissionWallTracker();
        tracker.Observe(1, ["\"src/a/One.cs\""]);   // quoted (as some messages embed it)
        tracker.Observe(2, ["src/a/One.cs"]);        // bare
    ...
    {
        const string jsonTool = "python3 -m json.tool out.json";
        var tracker = new PermissionWallTracker();
        tracker.Observe(1, [jsonTool], refusedCommands: [jsonTool]);   // attempt 1 — refused
        tracker.Observe(2, [jsonTool], refusedCommands: [jsonTool]);   // attempt 2 — refused again: a repeat

        // Left there, this is a halt — and correctly so, while the agent is still hitting it.
        Assert.True(tracker.ShouldHalt().Halt);

        tracker.Observe(3, []);                                         // attempt 3 — refused NOTHING
    ...
    {
        var tracker = new PermissionWallTracker();
        tracker.Observe(1, ["src/a/One.cs", ".claude/x.md"]);   // attempt 1
        tracker.Observe(2, ["src/a/One.cs", ".claude/x.md"]);   // attempt 2 → One.cs repeats, and the .claude/ wall is hit again
    ...
    public void ARefusedCommandThatNamesAClaudePath_IsNotTheStructuralWriteWall()
    {
        ...
        tracker.Observe(1, [grep], refusedCommands: [grep]);   // attempt 1
    ...
    public void ACommandRefusedOnTwoAttempts_IsARepeatedCommand_Verbatim_NotARepeatedPath()
    {
        ...
        tracker.Observe(1, [selfCheck], refusedCommands: [selfCheck]);   // attempt 1
        tracker.Observe(2, [selfCheck], refusedCommands: [selfCheck]);   // attempt 2
    ...
    public void PathsRefusedBesideACommand_AreStillPaths()
    {
        ...
        tracker.Observe(1, ["src/locked/Protected.cs", ".claude/agents/x.md", command], refusedCommands: [command]);
        tracker.Observe(2, ["src/locked/Protected.cs", ".claude/agents/x.md", command], refusedCommands: [command]);
    ...
    // ── #708 W5: a target is the wall only on an attempt that refused it ───────────────────────────────

    /// <summary>
    /// The review's probe. A path refused on attempts 1 and 2 stayed "repeated" on attempt 3, which refused only a command, so
    /// attempt 3 could halt "write repeatedly refused — src/locked/Protected.cs" although it never touched that path. #534's own
    /// comment already claims the rule this pins: a repeated path halts on the attempt that RE-hits it.
    /// </summary>
    [Fact]
    public void ATargetRefusedOnlyOnEarlierAttempts_IsNotThisAttemptsWall()
    {
        const string command = "pwsh -NoProfile -File guardrails/01-check.ps1";
        var tracker = new PermissionWallTracker();

        tracker.Observe(1, ["src/locked/Protected.cs"]);
        tracker.Observe(2, ["src/locked/Protected.cs"]);
        tracker.Observe(3, [command], refusedCommands: [command]);

        PermissionWallDecision decision = tracker.ShouldHalt();
        Assert.Empty(decision.RepeatedPaths);
        Assert.False(decision.Halt, "attempt 3 refused one command once; the path it never touched is not its wall");
    }

    /// <summary>
    /// A transient pause re-runs an attempt under the SAME number, and the wall is observed before the pause check, so one
    /// attempt used to be counted twice and could halt as a "repeat" on its own. Refusals are kept per attempt number, as a set.
    /// </summary>
    [Fact]
    public void AnAttemptObservedTwice_AfterATransientPause_CountsOnce()
    {
        var tracker = new PermissionWallTracker();

        tracker.Observe(1, ["src/locked/Protected.cs"]);   // attempt 1, paused by a rate limit
        tracker.Observe(1, ["src/locked/Protected.cs"]);   // attempt 1, re-run under the same number
        Assert.False(tracker.ShouldHalt().Halt, "one attempt, however often it ran, is not a repeat");

        tracker.Observe(2, ["src/locked/Protected.cs"]);
        Assert.Equal(new[] { "src/locked/Protected.cs" }, tracker.ShouldHalt().RepeatedPaths);
    }
}
