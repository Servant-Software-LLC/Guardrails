...
- **Agent-CLI runners share one session loop (#764).** `StreamJsonCliSession` owns everything after the
  ...
  it reads each completed `tool_call`'s `result.rejected`, and pairs `started`/`completed` by exact `call_id` (fallback `toolCallId`) through `CursorCallPairing` (#778: open at the terminal result = abandoned refusal, named for both roles but merged into the wall lists and counted toward AllShellRefused for judges only (`CursorPromptRunner.Finish` sets the lists by role; the scanner keeps abandoned targets in `AbandonedBlockedWritePaths`/`AbandonedCommands`); open with no terminal result = `PromptResult.InFlightToolCalls`, never a refusal; the transcript renderer shares that pairing, so `Render` and `StreamingWriter` keep one small piece of cross-line state), feeding the wall tracker and the #452 counter, which is inert for cursor actions because no task-action caller sets `AbortAfterConsecutiveToolDenials`) and an optional per-run
