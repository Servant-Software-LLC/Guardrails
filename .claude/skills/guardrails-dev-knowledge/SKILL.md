...
- **Diagnostic codes**: GR10xx loading, GR20xx validation (`DiagnosticCodes.cs`);
  ...
  #274 Part C `driftPolicy` check in the M1 fold, #254/#269/#274) is TAKEN too. **GR2032–GR2034** (multi-wave:
  mixed layout / wave numbering / cross-wave `dependsOn`) and **GR2035** (ExpectedDurationNonPositive — a
  guardrail sidecar's optional `expectedDurationSeconds` progress hint ≤ 0, SSOT §4.1.1, issue #331) are
  TAKEN too. Next free: GR1010 / GR2036.
