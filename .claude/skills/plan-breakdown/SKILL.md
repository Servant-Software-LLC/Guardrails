...
**Key it on the CHANGED TEXT as well, not only the component (#754).** Both keys above — the task's test
FILTER and its `writeScope` COMPONENT — miss the commonest case of all: a task told to reword
**user-visible text** (a banner, a halt message, a CLI option description, a diagnostic string) in
ordinary command code that is no renderer, serializer, formatter or schema, so neither trigger fires. So
when a task's brief changes a user-visible string literal, **grep the repo for the literal being
changed** — the exact thing a human does after the gate fires — find the tests asserting it verbatim, and
put those files in that task's `writeScope`, saying so in its prompt. Same three outs as above: own it,
give it a dedicated re-baseline ancestor, or name it in the Step 7 report.

Measured cost of not doing it: plan 41's `16-implement-halt-text-and-interlock-wording` correctly
reworded the undelivered-work banner, but `UndeliveredWorkWarningTests.cs` asserts that banner
**verbatim, in two places** and was in **no task's** `writeScope` — grepping the whole plan folder for
that filename returned nothing. All 20 tasks went green, the entire DAG was spent, and the run then
halted at the **terminal gate** on the merged HEAD, where nothing downstream could repair it. The
harness's own ownership diagnosis was precise and correct; the gap is that it can only fire at the last
possible moment, after the full cost has been paid. A breakdown-time grep costs seconds.
...
- **Faked-seam ⇒ paired real-seam proof (#382 — passing but blind)** — does an `author-tests-*` task
  ...
  **6. The ledger — the artifact this analysis produces. Its FORMAT IS A CONTRACT: `/guardrails-review`
  ...
  - **proof, the OTHER half — the named file must DECLARE the seam back (#382 residual).** A path in this
    column is a claim about a file; the file has to make the matching claim, or the ledger and the folder
    are two documents that can drift apart in silence. So every emitted proof carries a mandated marker:
    `real-seam` in its **basename**, so an audit can ENUMERATE the proofs instead of recognising them, and
    a header line
    `# real-seam: <Component> -> <Dependency>  bucket=<E|C|U>` naming **the same seam as its row**, so T\*
    is recomputable without recovering types from prose. Before this, the folder declared neither — the
    only tells were an example filename and a template phrase, so a correct proof spelled any other
    reasonable way was invisible to the audit, and a review that found nothing read exactly like a review
    that had nothing to find. The two statements are deliberately independent: crossing them is what
    catches a fabricated line or a dropped file, and it is the referential check `GR2061` is reserved for
    (doc 18 §3.4). It is NOT a schema field — doc 18 §10 holds that for re-review.
...
- [ ] **Every generated `.ps1` guardrail's next two lines are `$ErrorActionPreference = 'Stop'` then, immediately after, `$PSNativeCommandUseErrorActionPreference = $false`, shown together as one opening block (design 38 §4.1a)** — `'Stop'` turns an engine-raised error into a hard failure instead of the pwsh default fail-open (#608); the native-command line is load-bearing, not optional: without it, on a box where that preference is `$true`, a non-zero `dotnet test` — the SUCCESS condition of every inverse TDD-red check — becomes a terminating error and aborts the check before it can report red. Measured `$false` by default on pwsh 7.6.5, so today it is belt-and-braces, but it is a preference, not a guarantee, and shown apart an author copies one line and not the other. Every guardrail also **ends on an explicit `exit`** — the harness's abort-detecting shim relies on it, since a script that falls off its end after a failing native command would otherwise take that command's own exit code (all 900 committed guardrails already do this).
...
- [ ] (#509) The committed-pair requirement was applied at EVERY ALTITUDE, not only to task guardrails: `tasks/<id>/samples/`, `<wave>/samples/` and `<plan>/samples/<check>/{valid,invalid}/` are all legal, and the last two are where it matters most — a task guardrail that cannot fail passes one task, a wave-entry or plan-terminal gate that cannot fail waves the whole wave or the whole plan through. (A plan-root pair's halves are WORKSPACE TREES rather than files, because that is how a plan-root guardrail receives its subject.) Measured: a wave-entry gate's own invalid half caught four clauses that could never fail — the VALID half did not, because under an all-present tree everything passes either way.
...
- [ ] (#754) **Every task whose brief changes USER-VISIBLE TEXT owns the tests that pin it.** For each task told to reword a banner, a halt message, a CLI option description or a diagnostic string, the LITERAL itself was grepped across the repo and every test asserting it verbatim is either in that task's `writeScope` (and said so in its prompt), owned by a dedicated re-baseline ancestor, or named in the Step 7 report as deliberately left for later. This is the THIRD key on the cross-cutting-output rule, and the one with no component to key on: the FILTER key (#455) and the COMPONENT key (#541) both need a renderer / serializer / formatter / schema before they fire, and ordinary command code is none of those — so a plain reword passes both. Measured: plan 41's `16-implement-halt-text-and-interlock-wording` correctly reworded the undelivered-work banner while `UndeliveredWorkWarningTests.cs` pinned that banner **verbatim in two places** and sat in no task's `writeScope` — 20/20 tasks green, the entire DAG spent, then a **terminal-gate** halt on the merged HEAD that nothing downstream could repair. The harness's ownership diagnosis was exact; it simply cannot fire until the cost is already paid, and a breakdown-time grep costs seconds.
