...
1. **Initial breakdown.** Author + `/guardrails-review` **wave 1** only. Leave **just `wave-02-greet/`** as a
   stub — declared dir, **empty `tasks/`**, plus an **auto-seeded `brief.md`** drawn from the plan's
   **"Stage 2 (Wave 2) — Generate and check the greeting"** section (the wave's intent: run `out/greet.ps1`
   for the name in `out/config.json` → `out/greeting.txt`, then write `out/report.md` quoting it — the
   upstream artifacts it builds on, not its tasks). One wave visible ahead. **Do not** also stub
   `wave-03-publish/` (that is the #365 mistake in the other direction — see the negative example).
   `guardrails graph` shows `wave-01` + a `wave-02` stub node.
2. **Run → auto-break-down wave 2 at the checkpoint.** `guardrails run waved-hello3` runs wave 1, then reaches
   the wave-2 checkpoint; because the stub carries the **seeded `brief.md`**, the harness **auto-fires
   `plan-breakdown`** against it plus the **materialized integration worktree**
   (`<worktreeRoot>/<runId>/_integration` — the user's checkout stays read-only), `autoBreakdown` default-on.
   That breakdown does step 3, then the run **halts at `BreakdownComplete`** for the human review gate.
...
3. **What the fired breakdown does — author wave 2, then re-stub + re-seed wave 3.** Auto-fired by default (or
   ...
   create the next stub `wave-03-publish/` (dir + empty `tasks/`) and auto-seed its `brief.md`** from the
   plan's **"Stage 3 (Wave 3) — Publish"** section (its intent: build `out/published.md` from Stage 2's
   materialized `out/report.md`), then **regenerate the diagram** — restoring the one-ahead invariant (#365).
   `guardrails graph` now shows `wave-01`, `wave-02`, and a fresh `wave-03` stub node. The seeded `brief.md`
   is what makes the run **auto-break-down** wave 3 at its checkpoint by default (`autoBreakdown` default-on,
   SSOT §14.4/§14.10) — still halting for `/guardrails-review` — instead of honest-halting for a manual
   re-invocation. Had the plan carried **no identifiable Stage-3 section**, the skill would seed a **minimal
   template** `brief.md` (the wave title + a `> TODO: describe this wave's intent` placeholder) and flag it in
   the report for the human to flesh out — never a brief-less stub.
...
5. **Resume → auto-break-down wave 3 at the checkpoint.** `guardrails run waved-hello3` again — cross-wave
   resume skips wave 1, drains wave 2, then reaches the `wave-03-publish` checkpoint step 3 seeded and
   **auto-fires the wave-3 breakdown** (step 6), halting at `BreakdownComplete`. Without that re-stub the run
   would instead have completed at wave 2's terminal gate as if the plan were done (the #365 bug).
6. **What the wave-3 breakdown does — the FINAL wave, no re-stub.** It JIT-authors `wave-03-publish/tasks/`
   against the now-materialized `out/report.md`. Stage 3 is the last planned stage, so it **creates no stub
   after it** and regenerates the diagram (three authored waves, zero stubs). Then `/guardrails-review` it,
   `mark-reviewed`.
7. **Run → resume → complete.** `guardrails run waved-hello3` drains wave 3 and reaches the **terminal gate**
   (wave 3's exit gate on the fully-merged HEAD) — the plan is done. The JIT auto-breakdown loop ended exactly
   once, at the final wave, with no leftover stub.
...
>   not-yet-existing artifacts, it would be a JIT stub with an auto-seeded `brief.md` (run → auto-break-down
>   wave 2 at the checkpoint by default → halt for `/guardrails-review` → resume; a brief-less stub would
>   instead honest-halt for a manual re-invocation).
...
- **Leaving a JIT stub brief-less by default (the REVERSED rule, #360/§14.10).** The old doctrine created a
  *bare* stub and told the human to add a `brief.md` themselves. That is no longer the default: with
  auto-breakdown default-on, a brief-less stub **honest-halts for a manual re-invocation** instead of
  auto-breaking-down, defeating the whole point. Auto-seed the stub's `brief.md` from that wave's parent-plan
  section (or a flagged minimal template when the plan has no identifiable section) every time — one stub, one
  seeded brief. A human may still edit the seeded brief, or delete it to deliberately force a manual
  re-invocation, before the checkpoint is reached.
- **Seeding the WHOLE plan as the brief (not just that wave's section).** Dumping the entire parent plan
  `.md` into a stub's `brief.md` mis-scopes the auto-breakdown — the sub-invocation is supposed to break down
  ONE wave against the materialized upstream, so the brief must be that wave's INTENT only (the stage
  heading + its prose), never every stage. Scope the seed to the wave's own section.
