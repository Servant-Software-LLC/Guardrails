...
- **Failure detail lost to the tail (#179)**: for every guardrail that asserts a test suite
  **PASSES** (a `tests-pass` / `all-tests-pass` / `specific-tests-pass`, or a test driving a
  ...
  and retries blind (plan-0009 burned 12 attempts to `needsHuman`). Tell: no re-emit of the failure
  BLOCK after the `dotnet test` call. Fix: the capture → emit-full-log →
  ...
  feedback, costing attempts). **A re-emit that IS present but allowlist-shaped** — a `Select-String`
  or `Where-Object` over `[FAIL]` / `Error Message:` / `Assert.` / `Expected:` / `Actual:` /
  `Stack Trace:` — is the weaker legacy form and is worth its own **WEAK** note: measured, it drops a
  `DoesNotContain` failure's `String:`/`Found:` payload and every stack frame, and a hand-copied
  variant missing the `Assert.`/`Exception` branches also drops the assertion headline and a thrown
  test's only detail line, so `Error Message:` tails out followed by nothing (#608). Fix by switching
  to the block capture rather than by adding another branch to the alternation. Do NOT flag the INVERSE `tests-fail-on-stubs` /
...
- **Wave EXIT gate — GR2028 per wave + intermediate-wave union-safety (#254 / #125 / #165)**: the
  ...
  - **ANY** wave's exit gate carrying `scope:"integration"` — whole-suite or not — is a finding, and
    **not for the #125 reason.** The tag is **INERT at a wave root** (`validate` warns **GR2059**, #459):
    the per-union re-verify set is the task `<task>/guardrails/` folders plus the **PLAN-root**
    `<plan>/guardrails/` folder only (SSOT §4.3), so the file runs exactly once, on the merged HEAD at
    its own wave's exit (SSOT §14.3), tagged or not. It therefore **cannot** red-halt a correct partial
    merge; the defect is the mirror image — **the per-union invariant the author believed they authored
    does not exist**, and the plan merely LOOKS union-guarded. Severity: **WEAK** when the file is a
    genuine wave-exit postcondition carrying a dead tag (fix = drop the `scope` key; behaviour-identical);
    **BLOCKER** when the `catches:` line, the breakdown report, or the topology relies on per-union
    protection — the real union invariant is MISSING and must be authored at `<plan>/guardrails/`,
    keeping `scope:"integration"` there and made union-safe/CONDITIONAL (#125/#165). Never propose
    relocating a wave-exit gate to the plan root (that changes WHEN it runs), and do not pre-empt the
    open #459 contract question. A whole-build/suite check is **LOCAL** wherever it lives, and only the
    **LAST** wave's exit gate (which runs on the fully-merged HEAD) is the right home for a whole-suite
    LOCAL `tests-pass` — the whole-plan terminal boundary. Flag a plan-root `<plan>/guardrails/` that
    merely DUPLICATES the last wave's exit gate as a NIT (it is optional-additive, not a second terminal
    gate).
...
- **A task whose `writeScope` mixes WRITE MECHANISMS (#540)**: both a `.claude/**` path and a
  non-`.claude/**` path in one `writeScope`. The `.claude/` half can only be delivered through
  `needsHarnessWrite` (the tool-permission layer refuses a direct write, SSOT §9.3); the other half is
  written directly. One atomic attempt has to get BOTH right simultaneously.
  **Why every other probe in this pass walks past it.** Two paths, `dependsOn` fan-in of 1, no `maxTurns`
  bump — the task is SMALL by every measure the split-triggers and GR2042 read, and in prose it is
  arguably one deliverable ("record the new surfaces in the SSOT and the skill"). A full review pass on
  the plan that produced this found two BLOCKERs and did not see it, because every probe here evaluates
  guardrail strength or DAG edges; **none asks "can one attempt deliver all of this through the paths
  available to it?"**
  **What it costs.** A failed attempt is rolled back whole, so the task can never BANK the half it got
  right. Measured: 12 attempts, ~$4.66, never green, on the last task of a plan whose other seven passed
  in 8 attempts total. Attempt 7 wrote one half correctly and lost it with the rollback; by attempt 10
  that same half was failing again.
  Fix: split, one mechanism per task. `validate` warns (**GR2073**), so an existing plan carrying this
  shape reports itself — but a plan authored before that code, or run with an older tool, does not.
  **BLOCKER**: the task cannot be completed as written by any number of retries.
- **A committed sample pair whose guardrail ignores `GR_SUBJECT`, or a sample that is an ORPHAN (#559)**:
  the pre-DAG gate hands a sampled guardrail its subject as `argv[0]` AND as `GR_SUBJECT`. A guardrail that
  hardcodes its target instead scans the untouched repo for BOTH halves, so both exit the same way and the
  gate reports `ValidHalfFailed` — **halting the run before task one**, which is the most expensive place
  to learn it.
  **Read every sampled guardrail for the override**, and for a MULTI-subject guardrail check that
  `GR_SUBJECT` replaces the WHOLE subject list rather than being appended to it. Then check the file names:
  the verifier keys on the SECOND extension being exactly `.valid`/`.invalid`, so an extra case must read
  `X.invalid-<qualifier>.cs`; `X.<qualifier>.invalid.cs` parses as a lone half and is an `OrphanSample`.
  **Why a reading pass finds this and the author's own verification did not.** `plan-breakdown` used to
  prescribe author-time verification as "execute the guardrail against a hand-written valid sample", which
  an author naturally does by STAGING the sample into the real target path — proving the clauses and never
  touching the `GR_SUBJECT` path. The prescribed method passed while the enforced contract failed.
  Measured: plan 31 committed eight pairs, every one broken the same way, 11 findings at the gate.
  Cheapest confirmation, and the one to prefer over reading: **run `guardrails samples verify <folder>`**.
  **BLOCKER** — the run cannot start.
- **A TEST that synthesises a two-sided pair, with no distinctness assertion (#530)**: an
  ...
  codes*. **Read the body and the two halves together, and grade the OPERATOR, not the intent.** The
  measured case: `-match 'DEFECT'` against a `.valid` half reading *"a clean artifact, no defect here"* —
  PowerShell `-match` is case-INSENSITIVE, both halves exited 1, the pair discriminated nothing, and four
  tests could never pass. Its bash mirror is the opposite hazard: `grep -q` **is** case-sensitive, so the
  same fixture written both ways can discriminate on Linux and not on Windows.
  **What makes this a review finding rather than a run-time one** is that no gate downstream can see it.
  The task's own red census demands the tests be observed `Failed` — which a test that can never pass
  satisfies perfectly, red being the census's success condition — so the run goes green here and halts
  `needs-human` on the NEXT task, after the implementation agent has spent its retry budget on production
  code that was already correct. GR2055 (a guardrail that cannot pass for any input, #484) lints that
  shape in a *guardrail*; nothing lints it inside a test's own fixture, and this probe is the only place
  it is looked for.
  Fix: the fixture asserts **distinctness** — different exit codes, not the right ones; polarity belongs
  to the test's own assertions, and a fixture that inverts its pair on purpose is legitimate. Bind the
  subject the way the run binds it (`GR_SUBJECT` + `argv[0]` for a task pair, `GUARDRAILS_WORKSPACE` + cwd
  for a plan-root pair), or every body takes its no-subject early exit and the proof passes anything.
  **BLOCKER** — it costs a downstream halt and misdirects the agent that hits it.
  (Catalogue → "A test that SYNTHESISES a two-sided pair PROVES it discriminates"; the census's own
  boundary line is `stacks/dotnet.md §4.4`.)
...
- A script that fails to **parse** is a dead-end no retry can fix (#473) — the guardrail script is not in
  the task's write scope, so the agent cannot repair it and the task burns its whole budget to
  `needs-human`. So is one that crashes on a missing path, or whose `--filter` matches nothing.
  A mechanical `validate` lint backstops this: **GR2056** asks the language's own interpreter to PARSE
  the file (never to run it, so `validate` stays read-only and CI-safe). **Silence is not proof:** an
  absent interpreter or an unsupported language reports nothing at all — so a green `validate` does not
  retire this probe, and a `.sh` guardrail on a box with no `bash` was never checked.
...
*"if X is present"* half of a union-safe conditional (#125/#165), a ratcheting behaviour-manifest clause **on
a plan REGENERATED against a partially-landed tree** — but **an exception counts only if the script DECLARES
it**, and the ratchet exception counts only when the regeneration condition actually holds (a fresh plan has
no ratchet to be working; a nonzero manifest clause there is simply pre-satisfied). You are grading the
...
| every **numeric floor** × its own **filter's cardinality** (#484) | an arithmetic dead-end — a zero-match floor exceeding what the filter can ever select | **BLOCKER**. A mechanical `validate` lint backstops the provable slice: **GR2055** fires when the counted collection is demonstrably the one feeding the filter. It is deliberately CONSERVATIVE — an unrelated array or an unrelated threshold in the same script is not credited — so a green `validate` clears the provable case only, and the reconciliation is still yours |
...
- [ ] (#521 gap 1) Every REQUIRED-present clause whose guardrail's `catches:` line claims to prove a CALL, an invocation or wiring is anchored on one — `\.Member\s*\(`, WITH the trailing paren — not on a bare dotted reference. A dotted mention matches `nameof(Type.Member)` in a dead field, an XML doc reference, or a string literal, with ZERO invocations. The doctrine already existed in two places and the guardrail that was gamed was written by an agent with it loaded: **the word carrying the rule is "call"; the word that gets remembered is "dotted."** `validate` warns GR2074; an older tool or plan does not. Measured: a `nameof`-only mutant exited 0 while the committed valid sample still passed.
...
- [ ] Every `IFoo`/`FooImpl` pair has a wiring task + a composition-root guardrail that drives the REAL assembler (no guardrail that injects into the ASSEMBLER's own slot; whole-suite green does not stand in for wiring) (#120). The prohibition is SLOT-SPECIFIC — a #382 real-seam test injecting into the COMPONENT-UNDER-TEST's own constructor is REQUIRED, not a #120 violation (D12, same verb, different slot).
...
- [ ] (#610) Every test-author prompt's PINNED BEHAVIOUR LIST was read as a SET, not as a list of individually-correct items, and each pair asked: can both hold at once? The shape to hunt is an **unconditional guarantee** — "X ALWAYS happens to the last/first/only N" — colliding with a sibling that asserts "X NEVER happens to this item" **about that same item**. Measured on plan 36: sixteen pinned behaviours, four of them individually correct readings of the design, and no implementation could satisfy all four — the terminal phase always rescues `_lastEnqueued`, and three zero-attempt assertions each used the LAST emitted row. ~5 attempts, two hitting `maxTurns`, then a correct `needs-human`. Nothing downstream can recover: the implement task cannot touch the test file (the write-scope gate working as designed) and correctly refuses to weaken the guarantee. This is Probe C's shape one level up — Probe C reconciles clauses inside one guardrail FILE; this reconciles BEHAVIOURS inside one prompt — and it is a judgement call, not a lint, exactly as Probe C's prompt-to-guardrail axis is. NOT statically decidable, so there is deliberately no GR code: a mechanical gate here would look rigorous and certify nothing.
- [ ] (#601) Probe C's reconciliation was run ACROSS TASKS, not only inside each guardrail file: for every pair of tasks whose guardrails read the SAME file, one task's required-present literal was checked against the other's forbid-present patterns. Whichever runs second fails on work the first was required to do, and cannot fix it without breaking the first. Nothing else in the pipeline has a reading that spans two tasks — Probe C is per-file, #474 traces one datum to its carrier, and the write-scope check asks whether a scope covers a path (on the measured plan, it did). `validate` warns GR2076 where both paths are literal and identical, which is the conservative subset; the pairs it stays silent on (a computed path, a guardrail reading two files) are exactly the ones only a reader can settle. Measured on plan 35 task 13: three attempts and an overwatch intervention on a plan unsatisfiable the moment it was authored.
