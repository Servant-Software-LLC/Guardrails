...
public sealed class LiveRunObserver : IRunObserver, IAsyncDisposable
{
    ...
    public void SuppliedResourcesCommitted(IReadOnlyList<string> paths, string commit, string by)
    {
        lock (_gate)
        {
            // Design 40 §2 step 3 / design 41 §6: a run whose base changed underneath it must say so on
            // whichever surface the operator is watching — a silent base change is indistinguishable from a
            // harness bug when a later task behaves unexpectedly. NON-coalescing (like WaveStarting/
            // PlanHashMismatch): each supply is a distinct, rare event worth its own line, never folded into
            // a count. `by` is escaped like every other value on this surface.
            string escapedPaths = string.Join(", ", paths.Select(Markup.Escape));
            AppendNarrative(
                $"[bold]supplied by {Markup.Escape(by)}:[/] {paths.Count} resource(s) committed "
                + $"[grey]{Markup.Escape(commit)}[/] — " + escapedPaths);
        }
    }
