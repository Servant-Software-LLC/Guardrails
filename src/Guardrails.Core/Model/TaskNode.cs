...
public sealed record TaskNode
{
    ...
    /// <summary>
    /// Workspace-relative glob patterns the task may write. Loaded raw from the task.json
    /// <c>writeScope</c> field; <c>null</c> means absent (equivalent to universal <c>["**"]</c>
    /// at runtime). Two tasks may run concurrently only when their parsed scopes are disjoint.
    /// </summary>
