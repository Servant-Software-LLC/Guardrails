...
///
/// <para>Matching prefers STRUCTURED signals (an HTTP status 429/503/529) and falls back to a
/// small, explicit set of free-text phrases. The output-token-cap message
/// (<c>"…exceeded the 32000 output token maximum"</c>) is matched distinctly so the retry can
/// carry actionable feedback (issue #114). A miss is conservative: an UNrecognized error yields
/// <see cref="PromptFailureKind.Error"/> (consumes the budget, the status quo) — never a false
/// <see cref="PromptFailureKind.Transient"/> that could loop.</para>
/// </summary>
internal static class ClaudeSignalClassifier
{
    ...
    /// Output-cap takes precedence over transient (a cap is a distinct, actionable case); transient
