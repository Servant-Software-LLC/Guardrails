...
public sealed class OpenAiCompatPromptRunner : IPromptRunner
{
    ...
    private string ModelNotFoundRemedy(string model, string endpoint)
    {
        ...
        if (IsEngine(engine, "apple-fm"))
        {
            return $"Apple's system models are not downloadable — the `fm` stack serves a fixed set under Apple's " +
                   $"own ids, so `{model}` is either misspelled or not one this build serves. Run `fm --help` on the " +
                   "machine serving that endpoint. NB: `apple-fm` is macOS-only, and its OpenAI-compatible server is " +
                   "undocumented by Apple and beta as of macOS 27.";
        }

        return $"Make `{model}` available at {endpoint}. The block declares no `engine` hint, so there is no " +
               $"engine-specific command to suggest — add one ({SuggestableEngines(endpoint)}) and this " +
               "message names the exact command next time.";
    }

    /// <summary>
    /// The engine hints worth SUGGESTING to the operator of this block. <c>apple-fm</c> names a macOS-only
    /// stack, but the machine serving <c>endpoint</c> is not necessarily this one — a Windows operator
    /// pointing at a Mac across the LAN is the entire point of a separate inference box — so it is withheld
    /// only when the endpoint is LOOPBACK and this host is not macOS, the one case where the server is
    /// provably not a Mac. This selects SUGGESTION TEXT only; see <see cref="ServerIsProvablyNotMac"/>.
    /// </summary>
    ...
    /// <summary>
    /// True only when we KNOW the endpoint is served by a non-Mac: the URL is loopback (so the server is
    /// this machine) and this machine is not macOS. A remote endpoint returns false — unknown is not the
    /// same as no, and suppressing a valid suggestion is the worse error of the two.
    /// </summary>
