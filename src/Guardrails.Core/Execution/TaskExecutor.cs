...
public sealed class TaskExecutor : ITaskExecutor
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // --- scope enforcement: snapshot pre-action workspace state (M5) -----------------
        WriteScope writeScope = task.WriteScope is null
            ? WriteScope.Parse(["**"])
            : WriteScope.Parse(task.WriteScope);
        WorkspaceScopeSnapshot scopePreImage = WorkspaceScopeEnforcer.Snapshot(
            _plan.Workspace, writeScope, _plan.Config.EnforcementIgnore, _plan.PlanDirectory);
        ...
        // --- scope enforcement: revert out-of-scope writes (M5) --------------------------
        // Revert is applied first; then we fail the attempt if there were any violations.
        // The revert is idempotent — a subsequent attempt's pre-action snapshot completes
        // any partial revert from a cancellation mid-revert (§5.3).
        ScopeViolationResult scopeCheck = WorkspaceScopeEnforcer.RevertOutOfScope(
            _plan.Workspace, writeScope, scopePreImage, _plan.PlanDirectory);
        if (scopeCheck.HasOutOfScopeWrites)
        {
            string violatingList = string.Join(", ", scopeCheck.ViolatingPaths);
            string feedback = $"Attempt {attemptNumber} failed: out-of-scope writes detected and reverted.\n\n" +
                              $"The action wrote to workspace path(s) outside its declared writeScope:\n" +
                              string.Join("\n", scopeCheck.ViolatingPaths.Select(p => $"- {p}")) + "\n\n" +
                              "Fix the action so it only writes within its declared writeScope.";
            return _journaler.FailedAttempt(
                task, attemptNumber, startedAt, relativeLogDir, logDir, feedback, isFinal,
                ...
                new TaskResult
                {
                    ...
                    Outcome = TaskOutcome.ActionFailed,
                    ActionExitCode = action.ExitCode,
                    Summary = $"out-of-scope writes (reverted): {violatingList}"
                },
                costUsd: action.CostUsd);
        }
